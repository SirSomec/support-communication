...
export function MarketingCommunicationsPanel({ onToast }) {
  ...
  return <section className="marketing-settings-global">
    ...
    <form className="marketing-settings-panel-card marketing-settings-rules" onSubmit={saveSettings}><div className="marketing-settings-panel-title"><ShieldCheck size={19} /><div><h3>Согласия и тихие часы</h3><p>Правила применяются автоматически перед отправкой каждой кампании.</p></div></div><div><label>Тихие часы с<select value={settingsDraft.quietHoursStart} onChange={(event) => setSettingsDraft((value) => ({ ...value, quietHoursStart: event.target.value }))}>{Array.from({ length: 24 }, (_, hour) => <option key={hour} value={hour}>{String(hour).padStart(2, "0")}:00</option>)}</select></label><label>до<select value={settingsDraft.quietHoursEnd} onChange={(event) => setSettingsDraft((value) => ({ ...value, quietHoursEnd: event.target.value }))}>{Array.from({ length: 24 }, (_, hour) => <option key={hour} value={hour}>{String(hour).padStart(2, "0")}:00</option>)}</select></label></div><label>Текст согласия<textarea value={settingsDraft.consentText} onChange={(event) => setSettingsDraft((value) => ({ ...value, consentText: event.target.value }))} /></label><button className="primary-action" type="submit"><Check size={16} /> Сохранить правила</button></form>
  </section>;
}
