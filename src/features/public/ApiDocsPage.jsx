...
const navigationItems = [
  ...
  { id: "marketing", label: "Маркетинговые рассылки" },
      ...
      <div className="api-docs-layout">
        ...
        <div className="api-docs-content">
          ...
          <section className="api-docs-section" id="marketing">
            ...
            <div className="api-docs-info-row"><div><strong><code>POST /marketing/audiences/import-preview</code></strong><span>Сверяет до 100 000 строк по client ID, внешнему ID, нормализованному телефону и email. Принимает технические поля и распространённые заголовки таблиц: <code>Телефон</code>, <code>Номер телефона</code>, <code>Почта</code>, <code>Электронная почта</code>; числовые телефонные ячейки XLSX также поддерживаются. Неоднозначные строки возвращает для ручного выбора существующего клиента.</span></div><div><strong><code>GET /marketing/analytics/campaigns</code></strong><span>Возвращает сводку очереди, отправок, доставок, ошибок и исключений по кампаниям. Если канал не передаёт прочтения, API указывает <code>not_supported</code>, а не нулевое значение.</span></div></div>
            ...
            <div className="api-docs-info-row">
              ...
              <div><strong><code>PATCH /marketing/clients/:id/channel-restriction</code></strong><span>Устанавливает или снимает запрет рассылок по одному каналу. Запрет имеет приоритет над согласием и глобальной отправкой без согласия; изменение фиксируется в аудите.</span></div>
            </div>
            ...
            <div className="api-docs-callout">
              <ShieldCheck size={21} />
              <div><strong>Согласие и запреты проверяются для каждого канала</strong><p>При первом контакте платформа запрашивает согласие в соответствующем канале. Индивидуальный запрет полностью блокирует тестовые, персональные и массовые рассылки в этом канале независимо от согласия. Входящая CRM-синхронизация не создаёт клиентов и не заменяет доказательство согласия; кампанию по CRM-аудитории нельзя запустить, если последний успешный снимок старше 24 часов.</p></div>
            </div>
          </section>
