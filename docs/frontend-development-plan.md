...
2. Реализовано: общий `Modal`; `Toast`, `StatusBadge`, `ToolbarSearch`, `SegmentedControl`, `EntityTable`, `Permission` и `Modal` уже вынесены в `src/ui.jsx`, Toast/StatusBadge/Permission/ChannelBadge/ToolbarSearch CSS — в `src/ui.css`, `DialogActionMenu` и `AuditTimeline` — в `src/features/dialogs/*`.
