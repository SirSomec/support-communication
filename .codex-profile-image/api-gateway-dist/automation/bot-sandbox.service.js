...
import { AutomationRepository, createEmptyAutomationState } from "./automation.repository.js";
import { BotRuntimeService, effectiveTriggerRules } from "./bot-runtime.service.js";
...
export class BotSandboxService {
    ...
    connections;
    environment;
    constructor(automationRepository, options = {}) {
    ...
    async createSession(input) {
        ...
        if (!resolved.config.channels.includes(channel))
            throw new Error("bot_sandbox_channel_unsupported");
        const now = this.now();
        const session = {
            channel,
            context: {},
    ...
    async getSession(tenantId, scenarioId, sessionId) {
        const session = await this.sessions.find(tenantId, sessionId, this.now());
        if (!session || session.scenarioId !== scenarioId)
            throw new Error("bot_sandbox_session_not_found");
        return session;
    }
    async deleteSession(tenantId, scenarioId, sessionId) {
        await this.getSession(tenantId, scenarioId, sessionId);
        await this.sessions.delete(tenantId, sessionId);
    }
    async postMessage(input) {
        const text = String(input.text ?? "").trim();
        if (!text && input.quickReply === undefined)
            throw new Error("bot_sandbox_message_required");
        ...
        if (hasAiNode)
            await this.assertSandboxBudget(input.tenantId, now);
        ...
        const usageTokens = messages.reduce((sum, message) => typeof message.usageTokens === "number" ? (sum ?? 0) + message.usageTokens : sum, null);
        const events = [];
        if (failureCode)
            events.push(sandboxFailureEvent(failureCode));
        ...
        const retrieval = aiCalled || (executedNode?.type === "ai_reply" && result?.step.outcome === "ai_handoff_requested" && text)
            ...
            : { cache: "skipped", passages: [], tokensUsed: 0 };
        const trace = {
            aiCalled,
            consultationTurns: Number(result?.instance.context?.consultationTurns ?? session.context.consultationTurns ?? 0),
            latencyMs,
        ...
        if (result && !failureCode) {
            ...
            if (result.instance.status === "handoff")
        ...
        if (input.webhooksEnabled !== undefined)
            session.webhooksEnabled = Boolean(input.webhooksEnabled);
        this.appendTurn(session, turn, now);
        return { session: await this.sessions.save(session), turn };
    }
    async saveRegression(input) {
        const session = await this.getSession(input.tenantId, input.scenarioId, input.sessionId);
        if (!session.turns.length)
            throw new Error("bot_sandbox_session_empty");
        const run = {
    ...
    async assertSandboxBudget(tenantId, now) {
        ...
        if ((await this.sessions.sandboxUsage(tenantId, now)) + SANDBOX_WORST_CASE_TOKENS > budget)
            throw new Error("bot_sandbox_budget_exhausted");
    }
    appendTurn(session, turn, now) {
    ...
    async traceRetrieval(tenantId, scenario, query, scenarioId) {
        try {
    ...
    buildEphemeralRepository(session, resolved) {
        const scenario = {
            ...resolved.config,
        ...
        const instance = session.currentNodeId
            ? {
                attempts: 0,
    ...
    async resolveScenarioConfig(tenantId, scenarioId, mode, pinnedVersionId) {
        ...
        if (!scenario)
            throw new Error("bot_sandbox_scenario_not_found");
        ...
        if (resolvedMode === "published") {
            const version = (pinnedVersionId ? published.find((item) => item.versionId === pinnedVersionId) : undefined)
                ...
                ?? published[0];
            if (!version)
                throw new Error("bot_sandbox_published_version_not_found");
            return {
...
function extractBotMessages(result) {
    return result.step.sideEffects
        .filter((effect) => effect.kind === "message_delivery")
        .map((effect) => {
        const payload = effect.descriptor?.payload ?? {};
        return {
...
function sandboxHandoffNote(reason) {
    const notes = {
...
const sandboxWebhookStub = async () => new Response(JSON.stringify({ note: "sandbox webhook stub: real call skipped", sandbox: true }), {
    headers: { "content-type": "application/json" },
    status: 200
});
//# sourceMappingURL=bot-sandbox.service.js.map