...
- Базовые стили находятся в `src/styles.css`; admin Webhooks/API/Security стили настроек вынесены в `src/features/settings/settings.css`, chart/rescue/column controls отчетов — в `src/features/reports/reports.css`, а стили notification center — в `src/features/notifications/notifications.css`.
