...
| --- | --- | --- | --- |
| `api-gateway` | External REST API, request envelope, auth context handoff, operations readiness and tenant-scoped marketing orchestration | `operations_runtime_records`, `operations_postgres_restore_check_results`, `operations_object_storage_restore_check_results`, `marketing_access`, `marketing_api_keys`, `marketing_settings`, `marketing_audiences`, `marketing_audience_members`, `marketing_audience_syncs`, `marketing_audience_sync_events`, `marketing_templates`, `marketing_template_versions`, `marketing_campaigns`, `marketing_deliveries`, `marketing_usage_charges`, `marketing_usage_invoices`, `marketing_consents`, `marketing_channel_restrictions`, `marketing_consent_text_versions`, `marketing_audit_events` | Operations runtime persistence has Prisma parity for load-test, restore-check, dead-letter replay, migration rollback-check and idempotency descriptors; marketing campaigns, consent, per-channel client restrictions, CRM audience sync and billing state are owned by the API gateway marketing module. |
...
| `tenant-service` | Tenant catalog, tenant users, tenant status, workspace settings rules, workspace topic directory and tenant audit events | `tenants`, `tenant_users`, `tenant_audit_events`, `settings_rules`, `settings_rule_audit_events`, `workspace_topics` | Tenant and workspace settings persistence has Prisma parity; cross-service consumers use the tenant API contract. |
...
| `routing-sla-service` | Teams, queues, current shift roster, operator presence, routing rules, assignments, SLA timers, rescue jobs and routing outcomes | `teams`, `team_memberships`, `support_queues`, `current_shifts`, `operator_presence_intervals`, `routing_rules`, `queue_memberships`, `operator_capacities`, `routing_analytics_rows`, `routing_jobs`, `routing_state_snapshots` | Routing directory, current shift, presence and runtime state have Prisma parity; provider/runtime smoke remains outside this ownership map. |
...
| `platform-admin-service` | Service-admin audit, impersonation, audit exports, redactions, break-glass approvals, platform-wide service mail settings and service support tickets | `service_admin_audit_events`, `service_admin_audit_exports`, `service_admin_audit_redactions`, `service_admin_impersonations`, `break_glass_approvals`, `service_mail_settings`, `support_tickets`, `support_ticket_messages` | Phase 1 slice 1.5 and Phase 10 slices 10.6-10.8 for cross-surface hardening |
