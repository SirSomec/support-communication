...
import { appealAnchorTag, resolveOrForkAppealConversation } from "../conversation/appeal-lifecycle.js";
...
import { acknowledgeTelegramPhoneShare, requestTelegramPhoneIfMissing } from "./telegram-phone-collection.js";
const INTEGRATION_SERVICE = "integrationService";
export function loadTelegramWebhookConfig(env = process.env) {
...
export async function handleTelegramWebhookFromRoute(input, config = loadTelegramWebhookConfig()) {
    if (!config.enabled) {
    ...
    const tenantConnection = resolveTelegramTenantByWebhookSecret(telegramConnections, providedSecret);
...
function resolveLegacyTenantId(config, providedSecret) {
...
export async function resolveTelegramInboundConversation(input) {
    ...
    const resolved = await resolveOrForkAppealConversation({
        ...
        createMutation: (conversation, eventType = "conversation.created") => conversationCreatedMutation(conversation, eventType),
        providerConversationId: chatId,
        tenantId
    });
    if (!resolved)
...
async function knownTelegramPhone(repository, input) {
    ...
    for (const conversation of conversations) {
        if (resolveConversationTenantId(conversation) !== input.tenantId)
            continue;
        if (conversation.id !== input.anchorId && !conversation.tags.includes(anchorTag))
            continue;
...
async function updateTelegramConversationProfile(input) {
    ...
    await input.conversationRepository.saveConversationMutation(conversationCreatedMutation(conversation, "conversation.updated"));
    return conversation;
}
function normalizedTelegramPhone(value) {
...
function conversationCreatedMutation(conversation, eventType = "conversation.created") {
...
export async function telegramRoutingQueueId(repository, tenantId, connection) {
...
export function telegramConversationId(tenantId, botId, chatId) {
...
export function telegramTenantEventId(tenantId, botId, providerEventId) {
...
function parseTelegramUpdate(body) {
    ...
    if (!text && !attachments.length && !phone)
        return null;
...
export function telegramMessageAttachments(message) {
    const document = objectRecord(message.document);
    if (document)
        return [telegramAttachment(document, { fallbackName: "document", mimeType: stringValue(document.mime_type), type: "document" })];
    const photo = Array.isArray(message.photo)
        ? message.photo.filter((item) => Boolean(objectRecord(item))).at(-1)
        : undefined;
    if (photo)
        return [telegramAttachment(photo, { fallbackName: "photo.jpg", mimeType: "image/jpeg", type: "image" })];
    const media = [
        ["video", "video", "video"],
    ...
    for (const [field, fallbackName, type] of media) {
        ...
        if (value)
            return [telegramAttachment(value, { fallbackName, mimeType: stringValue(value.mime_type), type })];
    }
    return [];
}
function telegramAttachment(value, defaults) {
...
function telegramContactPhone(message) {
...
export async function resolveTelegramRatedTarget(repository, input) {
...
async function resolveRatedOperator(repository, conversation) {
...
export function parseTelegramQualityRating(body) {
    ...
    if (!match || !chatId || !callbackQueryId)
        return null;
...
export function parseTelegramCsatFeedbackDecline(body) {
    const callback = body.callback_query;
    if (!callback || typeof callback !== "object")
        return null;
    if (String(callback.data ?? "").trim() !== CSAT_FEEDBACK_NEW_APPEAL_CALLBACK)
        return null;
    const message = callback.message;
    const chat = message?.chat;
    const chatId = String(chat?.id ?? "").trim();
...
function deniedEnvelope(code, message) {
    return createEnvelope({
        service: INTEGRATION_SERVICE,
        operation: "receiveTelegramWebhook",
        status: "denied",
...
export function telegramWebhookPathFingerprint(tenantId, chatId) {
    return createHash("sha256").update(`${tenantId}:${chatId}`).digest("hex").slice(0, 16);
}
//# sourceMappingURL=telegram-webhook.route.js.map