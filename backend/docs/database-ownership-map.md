...
| `tenant-service` | Tenant catalog, tenant users, tenant status, workspace settings rules and tenant audit events | `tenants`, `tenant_users`, `tenant_audit_events`, `settings_rules`, `settings_rule_audit_events` | Tenant and workspace settings persistence has Prisma parity; cross-service consumers use the tenant API contract. |
