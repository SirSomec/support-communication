...
export async function handlePublicSdkPresenceHeartbeat(input) {
    ...
    if (!auth.allowed)
        return denied("heartbeatPublicSdkPresence", auth.code);
    const sessionId = limitedToken(input.body.sessionId, 160);
    if (!sessionId)
        return invalid("heartbeatPublicSdkPresence", "sdk_presence_session_id_required");
...
export async function handlePublicSdkPresenceDisconnect(input) {
    const auth = await resolvePublicApiRequest({ authorization: input.authorization, environment: input.environment,
        lookup: input.lookup, requiredScope: "clients:identify" });
    if (!auth.allowed)
        return denied("disconnectPublicSdkPresence", auth.code);
    const sessionId = limitedToken(input.body.sessionId, 160);
    const connectionId = String(auth.context.channelConnectionId ?? "").trim();
    if (!sessionId)
        return invalid("disconnectPublicSdkPresence", "sdk_presence_session_id_required");
    if (!connectionId)
        return denied("disconnectPublicSdkPresence", "sdk_connection_inactive");
...
export function scopedSdkPresenceHash(...parts) { return createHash("sha256").update(parts.join("\0")).digest("hex"); }
const scopedHash = scopedSdkPresenceHash;
function limitedToken(value, limit) { return typeof value === "string" ? value.trim().slice(0, limit) : ""; }
...
function safePath(value) {
    const text = limitedToken(value, 2048);
    if (!text || !text.startsWith("/"))
        return null;
    return text.split(/[?#]/, 1)[0]?.slice(0, 1024) || null;
}
function safeWebLocation(value) {
    const text = limitedToken(value, 4096);
    if (!text)
        return null;
    try {
        ...
        if (!/^https?:$/.test(url.protocol))
            return null;
        return { path: url.pathname.slice(0, 1024) || "/", url: `${url.origin}${url.pathname}`.slice(0, 2048) };
    }
    catch {
        return null;
    }
...
function denied(operation, code) {
    return createEnvelope({ service: "integrationService", operation, status: "denied",
        data: {}, error: { code, message: "SDK presence request is not authorized for an active SDK connection." } });
}
//# sourceMappingURL=public-sdk-presence.route.js.map