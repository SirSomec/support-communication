...
- Base tokens and responsive compatibility overrides remain in `src/styles.css`; app shell CSS moved to `src/features/app-shell/app-shell.css`, dialog workspace CSS to `src/features/dialogs/dialogs.css`, ProductScreen/MetricTile primitives to `src/ui.css`, workspace-section primitives to `src/features/workspace-sections.css`, settings access/channel/topic/employee/integration/SDK CSS to `src/features/settings/settings.css`, and the existing feature CSS split remains in clients/reports/notifications/dialogs/automation/quality/templates/visitors modules.
...
| Панель: онлайн, перерыв, активные, ожидание | Реализована; статусы операторов (6 статусов FR §9.4) записываются самим оператором, влияют на распределение и показываются с временем в текущем статусе и сводкой времени по статусам за день | Drill-down оператора, bulk actions |
...
- Completed split of `src/App.jsx`, remaining large `src/styles.css` blocks, and several large feature screens without behavior changes: `RouteLoading`, `DialogWorkspace`, app shell CSS, dialog workspace CSS, ProductScreen/MetricTile CSS, workspace-section CSS, settings access/channel/topic/employee/integration/SDK CSS, onboarding model/controls/step content, auth model/state panel, and automation bot model/draft factory now live in dedicated modules.
...
- [x] Статусы операторов (FR §9.4): селектор в шапке приложения, влияние на автораспределение и ручное назначение, статус и время в текущем статусе в панели смены, сводка времени по статусам за день (`/presence/me`, `/presence/team`, событие `operator.presence.updated`). Отдельные настройки перерывов (правила перехода) — впереди.
...
1. Completed: `App.jsx`, `styles.css`, and large feature screens have been split into submodules without behavior changes. Current structure includes app shell, `DialogWorkspace`, `RouteLoading`, onboarding model/controls/step content, auth model/state panel, automation model/draft factory, workspace-section CSS, app shell CSS, dialogs CSS, ProductScreen/MetricTile CSS, settings access/channel/topic/employee/integration/SDK CSS, plus previously extracted feature CSS layers. Future work in this area is incremental refactoring driven by concrete screen growth, not an open mandatory plan item.
...
- [x] Продолжить разнос `App.jsx`, `src/styles.css` и крупных feature-экранов на подмодули.
