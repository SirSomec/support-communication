...
describe("OpenAI-compatible chat provider", () => {
    ...
    it("maps timeout and failures to redacted errors without leaking connection secrets", async () => {
        const provider = createOpenAiCompatibleChatProvider({ ...connection(), maxRetries: 0, timeoutMs: 100 }, {
            fetch: async (_url, init) => new Promise((_resolve, reject) => init?.signal?.addEventListener("abort", () => reject(Object.assign(new Error("super-secret-key leaked"), { name: "AbortError" }))))
        });
        await assert.rejects(provider.complete({ messages: [{ content: "Hello", role: "user" }] }), (error) => error instanceof AiProviderError && error.code === "provider_timeout" && !error.message.includes("super-secret-key"));
    });
    it("does not retry a provider validation error", async () => {
        ...
        await assert.rejects(provider.complete({ messages: [{ content: "Hello", role: "user" }] }), (error) => error instanceof AiProviderError && error.code === "provider_error");
...
function jsonResponse(body) { return new Response(JSON.stringify(body), { headers: { "Content-Type": "application/json" }, status: 200 }); }
//# sourceMappingURL=openai-compatible-chat.provider.test.js.map