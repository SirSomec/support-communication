...
export function appealAnchorTag(anchorId) {
...
export function withConversationMetadata(conversation, patch) {
    return {
        ...conversation,
        metadata: {
...
export function ensureAppealAnchorTag(conversation, anchorId) {
...
export function resolveClosedAt(conversation) {
...
export function detectRepeatAppeal(closedConversation) {
...
export function recordClosedAppealHistory(conversation, closedAt) {
    const historyRow = [
        closedAt.slice(0, 10),
...
export function buildFollowUpAppeal(closedConversation, anchorId, providerConversationId) {
...
export function releaseProviderBindingForClosedAppeal(conversation) {
...
export async function resolveOrForkAppealConversation(input) {
    ...
    if (closedParent.id !== existing.id || closedParent.providerConversationId !== existing.providerConversationId) {
        await input.conversationRepository.saveConversationMutation(input.createMutation(closedParent, "conversation.updated"));
    }
...
async function findLatestAppealConversation(repository, tenantId, anchorId) {
...
function resolveSortTimestamp(conversation) {
...
function findClosedHistoryEntries(previous) {
...
function parseAppealHistoryDate(value) {
...
function defaultTopicForChannel(conversation) {
...
//# sourceMappingURL=appeal-lifecycle.js.map