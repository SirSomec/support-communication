...
export function validateUrlKnowledgeSourceConfig(input, options = {}) {
    ...
    if (parsed.protocol !== "https:")
        return { ok: false, code: "url_source_https_required" };
    if (parsed.username || parsed.password)
        return { ok: false, code: "url_source_credentials_forbidden" };
    ...
    if (!hostname || isForbiddenLiteralHostname(hostname))
        return { ok: false, code: "url_source_host_forbidden" };
    ...
    if (normalizedUrl.length > maxLength)
        return { ok: false, code: "url_source_too_long" };
    return { ok: true, config: { url: normalizedUrl }, hostname };
}
function normalizedMaximumLength(value) {
...
function isForbiddenLiteralHostname(hostname) {
...
function isReservedIpv4(address) {
    ...
    if (first === 100 && second >= 64 && second <= 127)
        return true; // shared address space
...
function isReservedIpv6(address) {
    const prefix = (bits) => address >> BigInt(128 - bits);
    ...
    if (prefix(32) === 0x20010db8n)
        return true; // documentation
    // IPv4-compatible and IPv4-mapped addresses must not bypass IPv4 checks.
    if (prefix(96) === 0n || prefix(96) === 0xffffn)
        return isReservedIpv4(Number(address & 0xffffffffn));
    return false;
}
//# sourceMappingURL=url-source-config.js.map