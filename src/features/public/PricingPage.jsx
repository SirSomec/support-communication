...
export function PricingPage({ onNavigateAuth = () => {}, onRequestDemo = () => {}, onStartTrial = () => {} }) {
  ...
  return <main className="public-page pricing-page">
    ...
    <section className="pricing-grid" aria-label="Тарифы Support Communication">{loading ? <p>Загрузка тарифов…</p> : tariffs.map((tariff) => {
      ...
      return <article className={`pricing-card${free ? " featured" : ""}`} key={tariff.id}><header><strong>{tariff.name}</strong><span>{free ? "Для старта" : enterprise ? "Для крупных команд" : "В месяц"}</span></header><div className="pricing-price">{free ? "Бесплатно" : enterprise ? "По запросу" : formatPrice(tariff.priceMonthly)}</div><p>{tariff.ownerOnly ? "Один оператор — владелец организации" : `До ${tariff.includedUsers} операторов`}</p><ul>{(tariff.features ?? []).map((feature) => <li key={feature}><CheckCircle2 size={16} />{featureLabels[feature] ?? feature}</li>)}</ul>{enterprise ? <button className="public-btn secondary" onClick={() => onRequestDemo({ planInterest: "enterprise", source: "pricing-enterprise", title: "Запрос Enterprise" })} type="button">Связаться с нами</button> : <button className={`public-btn ${free ? "primary" : "secondary"}`} onClick={() => onStartTrial({ plan: tariff.id, source: "pricing-card" })} type="button">{free ? "Начать бесплатно" : "Выбрать тариф"}<ArrowRight size={16} /></button>}</article>;
    })}</section>
  </main>;
}
