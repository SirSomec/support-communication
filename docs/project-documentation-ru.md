...
- показывает workspace автоматизации и консоль сценария (вкладки Обзор/Настройка/Знания/Тест/Версии/Результаты);
- валидирует bot flow;
- создает и обновляет bot scenarios; правки опубликованного сценария копятся в черновике следующей версии (draft overlay), версию можно откатить;
- публикует сценарии; блокирует публикацию AI-ответа без источника без передачи оператору;
- ведёт консультационный цикл (ai_reply остаётся активным между репликами) с лимитом ходов и распознаванием просьбы позвать человека;
- применяет **рамки ответов** (запрещённые/операторские темы, обязательность источника, порог поиска, редактируемый отказ, правила поведения) до и после вызова модели;
- запускает **живой тест-чат** в изолированных sandbox-сессиях (реальный runtime и AI, отдельный бюджет токенов, trace «как бот думал»);
- создает proactive rules;
- фиксирует handoff от бота к человеку;
- принимает feedback оператора по AI-ответу и ведёт очередь его ревью (без автоизменения знаний);
- отдаёт telemetry/alerts и учитывает pilot flag `ai_agents_v1`.

Управление знаниями бота вынесено в отдельный сервис/раздел **«Знания»** (`knowledge-sources`): источники (документ/URL/MCP), их жизненный цикл и одобрение, retrieval (лексический + live MCP), «вопросы без ответа». MCP-подключение запрашивает tenant, одобряет service-admin.
...
Подробнее для администраторов и ops: [bots-ai-user-guide-ru.md](bots-ai-user-guide-ru.md), [bots-ai-operations-runbook.md](bots-ai-operations-runbook.md), [bots-ai-security-privacy-review.md](bots-ai-security-privacy-review.md).
...
| `operator_presence_intervals` | Интервалы статусов оператора (онлайн, занят, перерыв и т.д.) для маршрутизации и учета времени в статусах |
...
| GET | `/api/v1/presence/me` | Текущий статус оператора и каталог статусов (FR §9.4) |
| PUT | `/api/v1/presence/me` | Установить свой статус (влияет на распределение) |
| GET | `/api/v1/presence/team` | Статусы команды и время в статусах за период (старший/администратор) |
