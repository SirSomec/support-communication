export const DEFAULT_AGENT_SESSION_POLICY = {
...
export function estimateSessionTokens(text) {
    return Math.max(1, Math.ceil(text.trim().length / 4));
}
export function isSessionExpired(state, now = new Date()) {
    return Date.parse(state.expiresAt) <= now.getTime();
}
export function mergeFacts(existing, incoming, policy = DEFAULT_AGENT_SESSION_POLICY) {
    const map = new Map();
    for (const fact of existing) {
        map.set(fact.key, fact.value);
    }
    const pairs = Array.isArray(incoming)
        ? incoming.map((fact) => [fact.key, fact.value])
...
export function applySessionUpdate(current, input, policy = DEFAULT_AGENT_SESSION_POLICY) {
    ...
    const recentTurns = [
        ...base.recentTurns,
        { at: nowIso, role: "user", text: userText },
        ...(assistantText ? [{ at: nowIso, role: "assistant", text: assistantText }] : [])
    ...
    const shouldCompact = next.turnCount >= policy.compactionTurnThreshold ||
...
export function compactSession(state, policy = DEFAULT_AGENT_SESSION_POLICY) {
...
export function formatSessionForPrompt(state) {
...
function emptySession(input, nowIso, policy) {
    return {
...
function sameScope(state, input) {
    return state.tenantId === input.tenantId && state.conversationId === input.conversationId;
}
function normalizeNullable(incoming, fallback, max) {
...
function deriveSummary(userText, intent) {
    return intent ? `Клиент продолжает сценарий ${intent}: ${clip(userText, 160)}` : clip(userText, 200);
}
function extractPreservedFacts(state) {
    const preserved = [];
    if (state.intent)
        preserved.push({ key: "lastIntent", value: state.intent });
    if (state.openQuestion)
        preserved.push({ key: "openQuestion", value: state.openQuestion });
    return preserved;
}
