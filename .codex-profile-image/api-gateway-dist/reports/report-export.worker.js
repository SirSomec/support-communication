...
import { buildConversationReportEventWatermark, filterReportConversations } from "./report-conversation-filters.js";
import { buildDialogTranscriptFile, buildDialogTranscriptSnapshot, dialogTranscriptFiltersFromJob, isDialogTranscriptExportJob, normalizeDialogTranscriptFormat } from "./report-dialog-transcripts.js";
export function serializeReportRowsAsCsv(input) {
    const header = input.columns.map((column) => escapeCsvCell(column.label)).join(",");
    const rows = input.rows.map((row) => input.columns.map((column) => escapeCsvCell(row[column.id])).join(","));
    return [header, ...rows].join("\r\n");
}
export function createReportObjectStoragePort(adapter) {
...
export function createDeterministicReportObjectStorageAdapter(options = {}) {
...
export function createLocalReportObjectStorageAdapter(options) {
...
export function serializeReportRowsAsJson(input) {
    return JSON.stringify({
        columns: input.columns.map((column) => ({
            id: column.id,
            label: column.label
        })),
        rows: input.rows.map((row) => Object.fromEntries(input.columns.map((column) => [column.id, row[column.id] ?? null])))
    }, null, 2);
}
export function serializeReportRowsAsXlsx(input) {
    const sheetRows = [
...
export async function writeReportExportObject(input) {
...
export async function createReportExportFileDescriptor(input) {
...
export async function executeCsvReportExport(input) {
...
export async function executeJsonReportExport(input) {
    return writeReportExportObject({
...
export async function executeXlsxReportExport(input) {
    return writeReportExportObject({
...
export async function executeReportExportWorkerOnce(input) {
...
function cloneReportObjectBody(body) {
    return Buffer.isBuffer(body) ? Buffer.from(body) : body;
}
function cloneStoredReportObject(object) {
    return {
        ...object,
...
function reportObjectBodySize(body) {
    return Buffer.isBuffer(body) ? body.length : Buffer.byteLength(body);
}
function resolveReportObjectPath(rootDir, objectKey) {
...
function reportExportColumns(job) {
...
async function reportExportSnapshot(repository, job) {
    ...
    const workspace = buildLiveReportWorkspace(filteredConversations, options);
    return {
...
export function reportSnapshotAt(job) {
...
function reportTimezoneOffset(value) {
