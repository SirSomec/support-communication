...
> **Актуализировано 13 августа 2026:** текущий прод работает непосредственно на RUVDS (`supportcom.ru` → `194.87.206.159`) через production Docker Compose и Caddy edge. API Gateway, frontend, PostgreSQL, Redis, MinIO, ClamAV и воркеры запущены на этом хосте; `/api/v1/ready` подтверждает все зависимости. Реверс-SSH-туннель, `supportcom-api-bridge.service` (socat), nginx-proxy-manager и Raspberry Pi не используются.
>
> **Историческая справка (аудит на 28 июля, заменённый текущей проверкой):** этап 9 (staging gate → production) **не пройден**, а фактически работающий прод-сервер (`supportcom.ru`, Raspberry Pi, OpenMediaVault/Debian 12, IP 95.31.2.38) развёрнут **не по этому плану**. Реальная схема: реверс-прокси/TLS — **nginx-proxy-manager**, а не Caddy edge; backend приходит на `127.0.0.1:4101` через **реверс-SSH-туннель** с удалённого хоста + `supportcom-api-bridge.service` (socat); отдельного compose-проекта `compose.production.yml`, воркеров и ClamAV на сервере нет. Как следствие:
> - security-заголовки Caddyfile (CSP/HSTS, блок `/api/v1/metrics`) в бою **не действуют** (этап 3);
> - бэкапы и offsite-хранение (этап 7) **не развёрнуты**, RPO = ∞;
> - observability/алертинг (этап 6) **отсутствует** — падения прода и проваленные бэкапы не уведомляют никого;
> - CI-артефакты (compose/Caddy/systemd) существуют как **целевые**, но на сервере не применены.
>
> Статусы 🟡 в таблице ниже отражают готовность артефактов в репозитории; они не заменяют проверку каждого требования на RUVDS. Полный снимок прежней инфраструктуры: [`infrastructure-audit-2026-07-28.md`](infrastructure-audit-2026-07-28.md).
