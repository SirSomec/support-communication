...
import { createMfaOtpRuntimeFromEnv } from "./mfa-otp.js";
import { createInviteMailDeliveryFromEnv, createServiceMailOverrideResolver } from "../mail/service-mailer.js";
...
export class SettingsEmployeeService {
    ...
    recoveryDelivery;
    inviteDelivery;
    employeeSettings = new Map();
    groups = new Map();
    hydratedGroupTenants = new Set();
    auditEvents = [];
    constructor(identityRepository = IdentityRepository.default(), teamDirectoryRepository = TeamDirectoryRepository.default(), recoveryDelivery, inviteDelivery) {
    ...
    async inviteEmployee(payload = {}, options = {}) {
    ...
    async updateEmployee(employeeId, payload = {}, options = {}) {
    ...
    async resetEmployeePassword(employeeId, payload = {}, options = {}) {
        const user = await this.identityRepository.findTenantUser(employeeId);
        if (!user) {
    ...
    async resetEmployeeMfa(employeeId, payload = {}, options = {}) {
        const user = await this.identityRepository.findTenantUser(employeeId);
        if (!user) {
    ...
    async deactivateEmployee(employeeId, payload = {}, options = {}) {
        const user = await this.identityRepository.findTenantUser(employeeId);
        if (!user) {
    ...
    async deleteEmployee(employeeId, payload = {}, options = {}) {
        const user = await this.identityRepository.findTenantUser(employeeId);
        if (!user) {
    ...
    async resendEmployeeInvite(employeeId, payload = {}, options = {}) {
        const user = await this.identityRepository.findTenantUser(employeeId);
        if (!user) {
    ...
    async validateLastActiveAdministrator(user, nextRoleKey, nextStatus, operation) {
        ...
        const activeAdmins = tenantUsers.filter((candidate) => candidate.id !== user.id
    ...
    async updateGroup(groupId, payload = {}, options = {}) {
        const tenantId = normalizeTenantId(payload.tenantId ?? options.tenantId);
        if (!tenantId) {
    ...
    async deleteGroup(groupId, payload = {}, options = {}) {
        const tenantId = normalizeTenantId(payload.tenantId ?? options.tenantId);
        if (!tenantId) {
    ...
    async applyGroupMembers(tenantId, groupId, requestedMemberIds) {
        const group = this.groups.get(groupKey(tenantId, groupId));
        if (!group) {
            return;
        }
        ...
        for (const employeeId of [...added, ...removed]) {
            ...
            const next = { ...settings, groupId: targetGroupId };
            await this.identityRepository.saveTenantUser({
                ...user,
                metadata: { ...(user.metadata ?? {}), employeeSettings: next }
            });
    ...
    ensureDefaultGroupsForTenant(tenantId) {
    ...
    syncGroupMember(groupId, employeeId, tenantId) {
    ...
    removeGroupMember(employeeId, tenantId) {
        for (const [key, group] of this.groups) {
    ...
    resolveGroupId(tenantId, requested, roleKey) {
...
function groupKey(tenantId, groupId) {
    return `${tenantId}:${groupId}`;
}
function employeeSettingsFromMetadata(metadata) {
...
function auditEvent(action, tenantId, targetId, reason, at = new Date().toISOString()) {
    return {
        action,
        at,
...
function notFoundEnvelope(operation, employeeId) {
    return createEnvelope({
        service: SERVICE,
        operation,
        traceId: identityTraceId(SERVICE, operation),
...
function validateEmployeeTenant(operation, user, expectedTenantId) {
...
function normalizeRoleKey(value) {
    ...
    if (["admin", "administrator", "owner", "администратор"].includes(raw))
        return "admin";
    if (["senior", "senior operator", "старший сотрудник", "lead"].includes(raw))
        return "senior";
    return "employee";
}
function parseRoleKey(value) {
    const raw = String(value ?? "").trim().toLowerCase();
    if (["admin", "administrator", "owner", "администратор"].includes(raw))
        return "admin";
    if (["senior", "senior operator", "старший сотрудник", "lead"].includes(raw))
        return "senior";
    if (["employee", "operator", "сотрудник"].includes(raw))
        return "employee";
    return null;
}
...
function roleNameFromKey(roleKey) {
    ...
    if (roleKey === "service_admin")
        return "Администратор сервиса";
    return "Сотрудник";
}
function roleDescriptionFromKey(roleKey) {
    if (roleKey === "admin")
        return "Полный доступ: настройки, команда, отчёты и интеграции.";
    if (roleKey === "senior")
        return "Диалоги, панель контроля, отчёты, качество и командные шаблоны.";
    return "Диалоги, клиенты и личные шаблоны ответов.";
}
function normalizeGroupId(value, roleKey) {
...
function passwordStatusFromUser(user) {
    if (user.status === "invited")
...
function normalizeMfaStatus(value) {
    return value === "reset_pending" ? "reset_pending" : "enabled";
}
function buildExceptions(user, settings) {
    const exceptions = [];
    if (normalizeMfaStatus(user.mfa) === "reset_pending")
        exceptions.push("MFA сброшена — подтвердится при следующем входе");
    if (!settings.sensitiveData)
        exceptions.push("Персональные данные клиентов скрыты");
    if (settings.canOverride)
        exceptions.push("Разрешено назначение сверх лимита чатов");
    return exceptions;
}
function slugify(value) {
    return value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "group";
}
//# sourceMappingURL=settings-employee.service.js.map