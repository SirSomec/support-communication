...
import { IdentityRepository, hashPasswordCredential, hashServiceAdminToken, isLegacyPasswordCredential, verifyPasswordCredential } from "./identity.repository.js";
...
import { selectTenantMembership, tenantMembershipsFromUsers } from "./identity-auth-flow.repository.js";
...
export class AuthService {
    ...
    identityRepository;
    mfaOtp;
    defaultRuntimeWiring;
    serviceAdminMfaOtpOverride;
    constructor(identityRepository = IdentityRepository.default(), mfaOtp, serviceAdminMfaOtp, operatorPresenceService) {
        this.operatorPresenceService = operatorPresenceService;
    ...
    async login({ email, mfaChallengeId, otp, password } = {}) {
    ...
    async startOidcLogin({ domain, providerId = "oidc-main", redirectUri } = {}) {
    ...
    async completeOidcCallback({ code, error, errorDescription, state } = {}) {
    ...
    async completeSamlAcs({ assertionExpiresAt, assertionId, audience, now = new Date(), providerId = "saml-main", requestId, subjectId } = {}) {
    ...
    async logout({ reason = "Service admin logged out", sessionId } = {}) {
    ...
    async loginTenantOperator({ email, mfaChallengeId, otp, password, tenantId } = {}, { forceMfa = false } = {}) {
        ...
        const effectiveTenantUser = membershipUsers.find((user) => user.tenantId === effectiveTenantId && user.status === "active");
    ...
    async upgradeLegacyPasswordCredential(password, credential) {
    ...
    async createAndDeliverMfaChallenge(email, runtime = this.mfaOtp) {
    ...
    async getTenantOperatorState({ sessionId } = {}) {
    ...
    async logoutTenantOperator({ sessionId, tenantId, userId } = {}) {
    ...
    async acceptInvite({ code, email, mfaChallengeId, otp, password } = {}) {
    ...
    async completeRecovery({ email, mfaChallengeId, otp, password, token } = {}) {
    ...
    async selectTenant({ email, tenantId, userId } = {}) {
        ...
        const selected = await selectTenantMembership({ email: normalizedEmail, tenantId: normalizedTenantId }, this.identityRepository);
        if (!selected) {
            return createEnvelope({
                service: SERVICE,
                operation: "selectTenant",
    ...
    async recordAuthFlowAudit(input) {
        const event = {
            action: input.action,
            actor: input.email,
            at: new Date().toISOString(),
...
function serializeTenantMemberships(memberships) {
...
async function seedDefaultAuthFlowFixtures(repository) {
...
const PARTIAL_SSO_UNAVAILABLE_MESSAGE = "OIDC/SAML sign-in is not fully implemented yet (token exchange and assertion signature validation are pending) and is disabled in this runtime. Set AUTH_ALLOW_PARTIAL_SSO_FLOWS=true only for contract testing.";
function isPartialSsoRuntimeAllowed() {
    if (["development", "test"].includes(currentNodeEnv())) {
        return true;
    }
    return String(process.env.AUTH_ALLOW_PARTIAL_SSO_FLOWS ?? "").trim().toLowerCase() === "true";
}
function shouldSkipTenantOperatorMfa() {
...
function buildOidcAuthorizationUrl(provider, { nonce, redirectUri, state }) {
...
function hashOidcValue(value) {
    return `sha256:${createHash("sha256").update(value).digest("hex")}`;
}
function isSamlAssertionReplayConflict(error) {
    const prismaError = error;
    return (error instanceof Error && /SAML assertion replay already exists/.test(error.message))
        || prismaError?.code === "P2002";
}
function resolveSessionStateDenial(session) {
    if (!session) {
...
function resolveTenantOperatorSessionDenial(session) {
    if (!session) {
