...
const inMemoryTransactionTails = new Map();
export class PlatformRepository {
    ...
    async runInTransaction(lockKey, operation) {
        ...
        return this.prismaClient.$transaction(async (transaction) => {
            ...
            await transaction.$queryRawUnsafe("SELECT pg_advisory_xact_lock(hashtextextended($1, 0))::text", normalizedLockKey);
    ...
    async readStateAsync() {
        ...
        const [runtimeRows, telemetryRows, healthRows, alertRoutingRows, featureFlagRuleRows, auditRows, outboxRows] = await Promise.all([
    ...
    async saveIncidentIdempotencyKeyAsync(record) {
        if (!this.prismaClient) {
    ...
    async saveFeatureFlagRuleAsync(rule) {
        if (!this.prismaClient) {
    ...
    async listFeatureFlagRulesAsync(filters = {}) {
        if (!this.prismaClient) {
    ...
    async saveTelemetrySampleAsync(sample) {
        if (!this.prismaClient) {
    ...
    async listTelemetrySamplesAsync(filters = {}) {
        if (!this.prismaClient) {
    ...
    async saveHealthRollupAsync(rollup) {
        if (!this.prismaClient) {
    ...
    async listHealthRollupsAsync(filters = {}) {
        if (!this.prismaClient) {
    ...
    async saveAlertRoutingRuleAsync(rule) {
        if (!this.prismaClient) {
    ...
    async listAlertRoutingRulesAsync(filters = {}) {
        if (!this.prismaClient) {
    ...
    saveAlertAcknowledgement(acknowledgement) {
        this.assertSyncRuntimeAvailable();
    ...
    async saveAlertAcknowledgementAsync(acknowledgement) {
        return this.saveRuntimeRecord("alertAcknowledgements", acknowledgement.idempotencyKey
            ? `ack:${acknowledgement.idempotencyKey}`
            : String(toJsonRecord(acknowledgement.auditEvent).id ?? `${acknowledgement.componentId}:${acknowledgement.acknowledgedAt}`), acknowledgement, acknowledgement.componentId);
    }
    saveIncidentCommunicationAttempt(attempt) {
        this.assertSyncRuntimeAvailable();
    ...
    async saveIncidentCommunicationAttemptAsync(attempt) {
    ...
    async listIncidentCommunicationAttemptsAsync(filters = {}) {
        const items = this.prismaClient
            ? await this.listRuntimeRecords("incidentCommunicationAttempts", filters.incidentId ? { filterKey: filters.incidentId } : {})
    ...
    saveIncidentCommunicationRetry(retry) {
        this.assertSyncRuntimeAvailable();
    ...
    async saveIncidentCommunicationRetryAsync(retry) {
    ...
    async listIncidentCommunicationRetriesAsync(filters = {}) {
        const items = this.prismaClient
            ? await this.listRuntimeRecords("incidentCommunicationRetries", filters.incidentId ? { filterKey: filters.incidentId } : {})
    ...
    saveIncidentCommunicationDeadLetter(deadLetter) {
        this.assertSyncRuntimeAvailable();
    ...
    async saveIncidentCommunicationDeadLetterAsync(deadLetter) {
        if (!this.prismaClient) {
            return this.saveIncidentCommunicationDeadLetter(deadLetter);
        }
        const existing = await this.findRuntimeRecord("incidentCommunicationDeadLetters", deadLetter.attemptId);
        if (existing) {
            return existing;
        }
        return this.saveRuntimeRecord("incidentCommunicationDeadLetters", deadLetter.attemptId, deadLetter, deadLetter.incidentId);
    }
    listIncidentCommunicationDeadLetters(filters = {}) {
    ...
    async listIncidentCommunicationDeadLettersAsync(filters = {}) {
        const items = this.prismaClient
            ? await this.listRuntimeRecords("incidentCommunicationDeadLetters", filters.incidentId ? { filterKey: filters.incidentId } : {})
    ...
    async savePlatformAuditRowAsync(row) {
        if (!this.prismaClient) {
    ...
    async listPlatformAuditRowsAsync(filters = {}) {
        if (!this.prismaClient) {
    ...
    async savePlatformOutboxRowAsync(row) {
        if (!this.prismaClient) {
    ...
    updatePlatformOutboxRowStatus(idempotencyKey, status, payloadPatch = {}) {
        this.assertSyncRuntimeAvailable();
        let updated;
        this.store.update((state) => {
            const current = normalizeState(state);
            ...
            return {
                ...current,
                platformOutboxRows: current.platformOutboxRows.map((item) => item.idempotencyKey === idempotencyKey ? updated : item)
            };
        });
        return clone(updated);
    }
    async updatePlatformOutboxRowStatusAsync(idempotencyKey, status, payloadPatch = {}) {
        if (!this.prismaClient) {
    ...
    listPlatformOutboxRows(filters = {}) {
    ...
    async listPlatformOutboxRowsAsync(filters = {}) {
        if (!this.prismaClient) {
    ...
    async findRuntimeRecord(collection, entityKey) {
        if (!this.prismaClient) {
            return clone(this.readState()[collection].find((item) => platformRuntimeEntityKey(collection, item) === entityKey));
        }
    ...
    saveRuntimeRecordSynchronously(collection, record) {
        switch (collection) {
            case "alertAcknowledgements":
                return this.saveAlertAcknowledgement(record);
            case "featureFlagOutbox":
                return this.saveFeatureFlagOutbox(record);
            case "featureFlags":
                return this.saveFeatureFlag(record);
            case "incidentCommunicationAttempts":
                return this.saveIncidentCommunicationAttempt(record);
            case "incidentCommunicationDeadLetters":
                return this.saveIncidentCommunicationDeadLetter(record);
            case "incidentCommunicationRetries":
                return this.saveIncidentCommunicationRetry(record);
            case "incidentIdempotencyKeys":
                return this.saveIncidentIdempotencyKey(record);
            case "incidents":
                return this.saveIncident(record);
        }
        throw new Error(`platform_runtime_collection_unsupported:${collection}`);
    }
}
function assertCompletePrismaPlatformClient(client) {
...
async function withInMemoryTransactionLock(lockKey, operation) {
...
function isPlatformRuntimeCollection(collection) {
    return [
...
function platformRuntimeEntityKey(collection, record) {
...
function platformRuntimeFilterKey(collection, record) {
    const value = toJsonRecord(record);
    switch (collection) {
        case "alertAcknowledgements":
...
function platformRuntimeRecordId(collection, entityKey) {
    return `platform_runtime_${collection}_${createHash("sha256").update(entityKey).digest("hex").slice(0, 24)}`;
}
function toPrismaTelemetrySampleCreateInput(sample) {
    return {
...
function toPrismaTelemetrySampleUpdateInput(sample) {
    const { id: _id, ...data } = toPrismaTelemetrySampleCreateInput(sample);
    return data;
}
function toTelemetrySample(row) {
    return {
...
function toPrismaHealthRollupCreateInput(rollup) {
    return {
...
function toPrismaHealthRollupUpdateInput(rollup) {
    const { id: _id, ...data } = toPrismaHealthRollupCreateInput(rollup);
    return data;
}
function toHealthRollup(row) {
    return {
...
function toPrismaAlertRoutingRuleCreateInput(rule) {
    return {
...
function toPrismaAlertRoutingRuleUpdateInput(rule) {
    const { id: _id, createdAt: _createdAt, ...data } = toPrismaAlertRoutingRuleCreateInput(rule);
    return data;
}
function toAlertRoutingRule(row) {
    return {
...
function toPrismaFeatureFlagRuleCreateInput(rule) {
    return {
...
function toPrismaFeatureFlagRuleUpdateInput(rule) {
    const { id: _id, createdAt: _createdAt, ...data } = toPrismaFeatureFlagRuleCreateInput(rule);
    return data;
}
function toFeatureFlagRule(row) {
    return {
...
function toFeatureFlagRuleStatus(status) {
    if (status === "off" || status === "on" || status === "gradual" || status === "guarded") {
        return status;
    }
    return "off";
}
function toPrismaPlatformAuditRowCreateInput(row) {
    return {
        action: row.action,
        actor: row.actor,
...
function toPlatformAuditRow(row) {
    return {
        action: row.action,
        actor: row.actor,
        actorName: row.actorName,
...
function toPrismaPlatformOutboxRowCreateInput(row) {
    return {
...
function buildDateRangeFilter(since, until) {
    const range = {};
...
function toPlatformMutationKind(kind) {
...
function platformAuditRowsEqual(left, right) {
