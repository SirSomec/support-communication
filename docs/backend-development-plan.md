...
Status 2026-06-29: implemented first gateway contract slice for operator workload, assignments, SLA pause/resume, rescue timers and rescue reports. Chat limits are server-enforced and override cannot bypass full operators. Routing conversations, operators, queues, assignment job descriptors, SLA timer jobs and rescue report rows have a JSON-backed durable routing repository and API Gateway runtime bootstrap via `ROUTING_STORE_FILE` for local mode. Routing rules, queue membership rows and operator capacity records also have tenant-owned Prisma/PostgreSQL schema, additive migrations and JSON/Prisma repository adapters behind `ROUTING_REPOSITORY=prisma`. Workload reads are side-effect free, rescue return descriptors use the `rescue-return` queue, rescue reports preserve the previous operator attribution after queue return/miss, and default local routing stores are isolated by service/environment/port. Update 2026-06-30: Phase 4 is complete; routing analytics rows cover assignment, transfer, rescue and auto-return runtime writes, Prisma persistence, workload aggregates and rescue report aggregates. Slice 4.4 review-hardening is also complete; rescue-return jobs preserve conversation ownership, claim/apply workers re-read current job state before mutation, resolveRescue rescue-return jobs carry worker-required fields, and partial routing repository seeds are type-aligned with runtime normalization. Update 2026-07-03: routing assignment/SLA/rescue job descriptors now persist through Prisma `routing_jobs`, Prisma routing mode hydrates/persists conversation/operator/queue state plus rescue report rows through the `routing_state_snapshots` table instead of the JSON fallback, SLA/rescue due-job workers claim jobs through repository-owned compare-and-set updates instead of stale list/save snapshots, SLA/rescue apply outcomes persist through repository-owned transaction hooks, apply hooks validate current job and conversation state, tenant-scoped rescue auto-return analytics resolve tenant ownership from conversation snapshots, and Prisma conflict tests cover rollback of job completion plus analytics side effects.
...
Status 2026-06-28: implemented first gateway contract slice for reports and exports: `/api/v1/reports/workspace`, `/api/v1/reports/exports`, `/api/v1/reports/exports/:jobId/retry`, `/api/v1/reports/exports/:jobId/file`. The slice covers `metrics/v1`, rescue report rows, idempotent export queue requests with request fingerprinting, retry guards and permission-aware file descriptors that do not expose object keys. Report export jobs and idempotency fingerprints now have a JSON-backed durable report repository and API Gateway runtime bootstrap via `REPORT_STORE_FILE`, so queued exports and retry state survive service instances. Update 2026-07-03: report persistence now supports `REPORT_REPOSITORY=prisma` for production-like runtime. Prisma mode stores metric definitions/versions/overrides, saved templates, idempotency keys, export jobs, query executions, export file descriptors, notification descriptors, scheduled digest descriptors and immutable retry audit events in PostgreSQL, with additive migration coverage and bootstrap through the shared repository selector.

Notification runtime update 2026-07-03: notification persistence now supports `NOTIFICATION_REPOSITORY=prisma` for production-like runtime. Prisma mode stores inbox notifications, tenant/user preferences, browser push subscriptions, delivery descriptors and immutable preference audit events in PostgreSQL, with additive migration coverage and bootstrap through the shared repository selector.
...
Status 2026-07-03 update: integrations now support `INTEGRATION_REPOSITORY=prisma` for production-like runtime persistence. Prisma mode stores public API keys/reveal state/rotation audit, rotation jobs, public demo request lead/audit/notification descriptors, webhook delivery/replay journals and audit events, security sessions, channel connections/events/audit events, and Telegram connection state. Runtime consumers for notification channel validation, Telegram polling/webhook ingress, and outbound Telegram dispatch use async repository readers instead of JSON fallback state.
...
Status 2026-06-28: implemented first gateway contract slice for automation, bot runtime descriptors, proactive rules and quality scoring: `/api/v1/automation/workspace`, `/api/v1/automation/bot-flow/validate`, `/api/v1/automation/bot-flows/validate`, `/api/v1/automation/bots/:scenarioId/publish`, `/api/v1/automation/bot-scenarios/:scenarioId/publish`, `/api/v1/automation/bot-scenarios/:scenarioId/test-runs`, `/api/v1/automation/proactive-rules`, `/api/v1/automation/handoff-events`, `/api/v1/automation/bot-handoffs`, `/api/v1/quality/workspace`, `/api/v1/quality/draft-score`, `/api/v1/quality/draft-scores`, `/api/v1/quality/ratings`, `/api/v1/quality/manual-reviews`. Bot publish scenarios, publish idempotency fingerprints, proactive rules and bot test-run descriptors now have a JSON-backed durable automation repository and API Gateway runtime bootstrap via `AUTOMATION_STORE_FILE`. Update 2026-07-03: automation persistence now supports `AUTOMATION_REPOSITORY=prisma` for production-like runtime. Prisma mode stores bot scenarios, runtime versions, publish audit rows, publish idempotency keys, bot test runs, proactive rules, execution windows, frequency caps, experiment assignments, delivery attempts, delivery idempotency keys and delivery attributions in PostgreSQL, with async proactive eligibility/planning helpers that avoid sync JSON fallback reads.
...
Status 2026-06-28: implemented first gateway contract slice for platform monitoring, incidents and feature flags: `/api/v1/platform/snapshot`, `/api/v1/platform/components/:componentId`, `/api/v1/platform/components/:componentId/acknowledgements`, `/api/v1/platform-monitoring/snapshot`, `/api/v1/platform-monitoring/components/:componentId`, `/api/v1/platform-monitoring/components/:componentId/acknowledgements`, `/api/v1/incidents`, `/api/v1/incidents/:incidentId`, `/api/v1/incidents/:incidentId/updates`, `/api/v1/feature-flags`, `/api/v1/feature-flags/:flagId/preview`, `/api/v1/feature-flags/:flagId`, `/api/v1/feature-flags/:flagId/internal-tests`. Update 2026-07-03: platform persistence now supports `PLATFORM_REPOSITORY=prisma` for production-like runtime. Prisma mode stores telemetry samples, health rollups, alert routing rules, feature flag rules, platform audit/outbox rows and platform runtime descriptors in PostgreSQL through dedicated platform tables plus `platform_runtime_records`.
...
- [x] Slice 4.6.routing-job-prisma: persist assignment, SLA and rescue job descriptors through the Prisma routing repository port.
- [x] Slice 4.6.runtime-snapshot-contract: add Prisma routing repository contracts for hydrating and saving runtime state snapshots without fallback.
- [x] Slice 4.6.runtime-snapshot-schema: add `routing_state_snapshots` schema and migration coverage for conversation/operator/queue state plus rescue report rows.
- [x] Slice 4.6.runtime-snapshot-prisma: hydrate API Gateway routing startup from Prisma snapshot cache and persist state transitions through Prisma-backed snapshot writes.
- [x] Slice 4.7.atomic-job-claim-contract: add Prisma and worker contracts for repository-owned compare-and-set job claims.
- [x] Slice 4.7.atomic-job-claim-runtime: wire SLA/rescue claim workers to `RoutingRepository.claimJob()` and reject stale claim races.
- [x] Slice 4.7.stale-apply-hardening: skip SLA apply transitions unless the repository-current job is still claimed.
- [x] Slice 4.7.prisma-stale-apply-hardening: skip SLA/rescue apply transitions when another Prisma-backed worker has already completed the job after local hydration.
- [x] Slice 4.7.snapshot-conflict-side-effect-hardening: make Prisma `saveState()` reject snapshot-version conflicts before writing side-table jobs, analytics rows, rules, memberships, or capacities.
- [x] Slice 4.8.worker-delegation: route SLA/rescue apply workers through repository-owned apply hooks instead of manual `readState()`/`saveState()` mutation.
- [x] Slice 4.8.prisma-transaction-boundary: persist SLA/rescue apply outcomes in one Prisma transaction covering job completion, snapshot state, descriptors and analytics rows.
- [x] Slice 4.8.conflict-rollback-hardening: cover snapshot-version conflicts so Prisma rescue-return apply does not leave completed jobs or analytics rows behind.
- [x] Slice 4.8.current-state-tenant-hardening: validate repository-current SLA conversation ownership/state and resolve rescue auto-return analytics tenant from the conversation snapshot.
