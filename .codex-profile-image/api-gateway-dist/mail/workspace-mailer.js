...
import { composeMailMessage, sendSmtpMail, smtpMailAddress } from "./smtp-transport.js";
...
export function mailSecretStore(environment = process.env) {
 ...
 */
export function transportConfigFromSettings(record, environment = process.env) {
    let auth;
    if (record.username) {
        if (!record.secret) {
...
export function loadEnvironmentTransportConfig(source = process.env) {
 ...
 */
export async function resolveTenantTransportConfig(tenantId, options = {}) {
 ...
 */
export function createInviteMailDeliveryFromEnv(source = process.env, options = {}) {
...
function buildInviteMail(input, source) {
...
function inviteFingerprint(input) {
...
export function applicationBaseUrl(source = process.env) {
...
function logMailFallback(reason, tenantId, error) {
...
function environmentEncryption(source) {
...
//# sourceMappingURL=workspace-mailer.js.map