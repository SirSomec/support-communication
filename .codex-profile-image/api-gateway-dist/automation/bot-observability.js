import { METRIC_BUCKETS, metricsRegistry, sanitizeMetricLabel } from "@support-communication/observability";
/** Bot/AI automation metrics with bounded labels (tenant/scenario/reason codes only). */
export function botMetrics(registry = metricsRegistry()) {
    return {
...
export function recordBotSourceError(input) {
...
export function recordBotAiRequest(input) {
    ...
    if (typeof input.latencyMs === "number")
        metrics.aiLatencyMs.observe(baseLabels(input), Math.max(0, input.latencyMs));
...
export function recordBotHandoff(input) {
...
export function recordBotPublishFailure(input) {
...
function baseLabels(input) {
    return {
        scenario_id: sanitizeMetricLabel(input.scenarioId ?? "none"),
        tenant_id: sanitizeMetricLabel(input.tenantId)
    };
}
//# sourceMappingURL=bot-observability.js.map