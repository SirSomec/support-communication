...
export class KnowledgeRetrievalController {
  ...
  retrieve(@Body() body: { mode?: string; query?: string; scenarioId?: string; sourceIds?: string[]; tokenBudget?: number }, @Req() request: TenantOperatorRequest & ServiceAdminRequest) { return this.service.retrieveScenario({ ...(body ?? {}), tenantId: request.tenantOperatorContext?.tenantId ?? request.serviceAdminContext?.currentTenantId ?? "" }); }
}
