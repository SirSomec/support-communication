...
export class LlmKnowledgeSearchService {
    connections;
    environment;
    usage;
    providerFactory;
    constructor(connections = AiConnectionRepository.default(), environment = process.env, usage = AiUsageRepository.default(), providerFactory = createOpenAiCompatibleChatProvider) {
    ...
    async search(input) {
        ...
        if (!connection)
            throw new Error("llm_retrieval_connection_not_ready");
        ...
        const worstCaseTokens = Math.min(input.corpus.tokenEstimate + 500, connection.limits.monthlyTokenBudget ?? input.corpus.tokenEstimate + 500);
        ...
        try {
            ...
            const request = usesExplicitPromptCacheBreakpoints(String(connection.retrievalModel))
                ? {
                    maxTokens: 300,
            ...
            await this.usage.recordUsage(input.tenantId, connection.id, completion.usage.totalTokens ?? Math.ceil((input.corpus.promptText.length + input.query.length) / 4));
            return {
 ...
 */
export function selectPassages(content, chunks) {
    ...
    const items = Array.isArray(parsed?.chunks) ? parsed.chunks : [];
    ...
    const passages = [];
    const seen = new Set();
    const push = (chunk, score) => {
        if (seen.has(chunk.chunkId) || passages.length >= 8)
            return;
...
//# sourceMappingURL=llm-knowledge-search.service.js.map