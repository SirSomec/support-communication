...
import { makeEphemeralPlatformMutationIdempotencyKey, persistPlatformAlertMutationAsync } from "./platform-audit-outbox.js";
...
export class PlatformMonitoringService {
    ...
    async fetchPlatformSnapshot(filters = {}) {
        ...
        const sloBurnRate = latestTelemetryValue(readModel.telemetrySamples.filter((sample) => componentIds.has(sample.componentId)), ["slo_burn_rate", "sloBurnRate"]);
    ...
    async saveAlertRoutingRule(payload) {
    ...
    async ingestTelemetrySample(payload) {
        const request = payload ?? {};
    ...
    async writeHealthRollup(payload) {
        const request = payload ?? {};
        const component = await this.resolveWritableComponent(request.componentId ?? "");
        if (!component) {
        ...
        if (!isFiniteNumber(request.availability) ||
    ...
    async acknowledgeComponentAlert(payload) {
        const request = payload ?? {};
...
function syntheticPlatformComponent(componentId) {
    return {
        dependencies: [],
        errorRate: null,
        id: componentId,
...
function averageUptime(components) {
...
function alertRoutingRuleId() {
    return `alert-route-${randomUUID()}`;
}
function clone(value) {
    return JSON.parse(JSON.stringify(value));
}
function componentStatusMatches(component, status) {
...
function derivePlatformComponents(catalogComponents, incidents, telemetrySamples, healthRollups) {
...
function latestTelemetryValue(samples, metricKeys) {
...
function platformComponentStatus(healthStatus, activeIncident, catalogStatus) {
...
function fingerprintPlatformAlertNotification(payload) {
...
function telemetrySampleId() {
    return `telemetry-sample-${randomUUID()}`;
}
function telemetrySampleMetric(sample) {
    return {
...
function telemetrySampleRetention(sample) {
    return {
...
function statusPageSync(scope, target) {
    return {
...
function platformAlertNotificationOutboxId(idempotencyKey, routeId) {
...
function platformAlertNotificationOutboxIdempotencyKey(idempotencyKey, routeId) {
    return `platform-outbox:alert-notification:${idempotencyKey}:${routeId}`;
}
//# sourceMappingURL=platform-monitoring.service.js.map