...
export class RoutingService {
    ...
    queues;
    rescueReportRows;
    constructor(routingRepository = RoutingRepository.default(), canonicalWorkload, canonicalConversations, canonicalTeams, operatorPresence = OperatorPresenceRepository.default()) {
        this.routingRepository = routingRepository;
        this.canonicalWorkload = canonicalWorkload;
        this.canonicalConversations = canonicalConversations;
        this.canonicalTeams = canonicalTeams;
    ...
    async fetchWorkload(filters = {}, context = {}) {
        return this.withRoutingState(() => this.fetchWorkloadUnlocked(filters, context));
    }
    async fetchWorkloadUnlocked(filters = {}, context = {}) {
        ...
        const operators = operatorSource
            .map((operator) => operatorProjection(operator, channel, findCapacityForOperator(capacities, operator.id, channel), canonical
    ...
    async createAssignment(payload, context = {}) {
        return this.withRoutingState(() => this.createAssignmentUnlocked(payload, context));
    }
    async createAssignmentUnlocked(payload, context = {}) {
        const tenantId = requiredTenantId(context);
        if (!tenantId) {
        ...
        if (assignedTeamId)
            conversation.teamId = assignedTeamId;
    ...
    async simulateAssignment(payload, context = {}) {
        return this.withRoutingState(() => this.simulateAssignmentUnlocked(payload, context));
    }
    async simulateAssignmentUnlocked(payload, context = {}) {
        const tenantId = requiredTenantId(context);
        if (!tenantId) {
    ...
    async autoAssignConversation(conversationId, context = {}) {
        return this.withRoutingState(() => this.autoAssignConversationUnlocked(conversationId, context));
    }
    async autoAssignConversationUnlocked(conversationId, context = {}) {
        const tenantId = requiredTenantId(context);
        if (!tenantId)
            return tenantContextRequiredEnvelope("autoAssignConversation");
        ...
        if (!candidate) {
            ...
            if (conversation.persistedStatus !== "queued" && conversation.persistedStatus !== "closed") {
                const returned = await this.returnConversationToQueue(clone(conversation), tenantId, "No eligible operator available for automatic assignment", context, conversation);
                if (returned.status === "ok") {
                    queuedConversation = returned.data.conversation;
                }
            }
            return createEnvelope({
                service: ROUTING_SERVICE,
    ...
    async previewRedistribution(payload, context = {}) {
        return this.withRoutingState(() => this.previewRedistributionUnlocked(payload, context));
    }
    async previewRedistributionUnlocked(payload, context = {}) {
        const tenantId = requiredTenantId(context);
        if (!tenantId) {
    ...
    async commitRedistribution(payload, context = {}) {
        return this.withRoutingState(() => this.commitRedistributionUnlocked(payload, context));
    }
    async commitRedistributionUnlocked(payload, context = {}) {
        const tenantId = requiredTenantId(context);
        if (!tenantId) {
        ...
        const canonicalById = new Map(this.conversations
            .filter((conversation) => conversation.tenantId === tenantId && "persistedStatus" in conversation)
    ...
    async pauseSla(payload, context = {}) {
        return this.withRoutingState(() => this.pauseSlaUnlocked(payload, context));
    }
    async pauseSlaUnlocked(payload, context = {}) {
        const tenantId = requiredTenantId(context);
        if (!tenantId) {
    ...
    async startRescue(payload, context = {}) {
        return this.withRoutingState(() => this.startRescueUnlocked(payload, context));
    }
    async startRescueUnlocked(payload, context = {}) {
        const tenantId = requiredTenantId(context);
        if (!tenantId) {
    ...
    async resolveRescue(payload, context = {}) {
        return this.withRoutingState(() => this.resolveRescueUnlocked(payload, context));
    }
    async resolveRescueUnlocked(payload, context = {}) {
        const tenantId = requiredTenantId(context);
        if (!tenantId) {
        ...
        const pendingRescueReturnJob = (await this.routingRepository.listJobs()).find((job) => job.queue === "rescue-return"
            && job.action === "return_to_sla_queue"
            && job.conversationId === conversation.id
            && job.tenantId === tenantId
            && (job.status === "pending" || job.status === "claimed"));
        const rescueReturnJob = pendingRescueReturnJob
            ? {
    ...
    async fetchRescueReport(filters = {}, context = {}) {
        return this.withRoutingState(() => this.fetchRescueReportUnlocked(filters, context));
    }
    async fetchRescueReportUnlocked(filters = {}, context = {}) {
        const tenantId = requiredTenantId(context);
        if (!tenantId) {
    ...
    normalizeRedistributionPayload(payload, operation) {
    ...
    async buildRedistributionPlan(payload, tenantId) {
        ...
        const plannedOperatorLoad = new Map();
        const plan = [];
        const capacityConflicts = missingQueues.map((queue) => ({
    ...
    async buildAssignmentCandidates(conversation, tenantId, plannedOperatorLoad = new Map(), lastAssignedAtByOperator) {
     ...
     */
    async readLastAssignedAtByOperator(tenantId) {
    ...
    async returnConversationToQueue(conversation, tenantId, reason, context, canonicalConversation) {
        const previousOperatorId = conversation.operatorId ?? null;
        const previousStatus = conversation.status;
        conversation.operatorId = undefined;
        conversation.status = "queued";
    ...
    moveOperatorAssignment(previousOperatorId, nextOperatorId, tenantId) {
    ...
    moveOperatorRescueActive(operatorId, delta, tenantId) {
        if (!operatorId) {
            return;
        }
        const operator = this.findOperatorForTenant(operatorId, tenantId);
    ...
    moveQueueWaitingToActive(channel, tenantId) {
    ...
    async listActiveQueueMemberships(channel, tenantId) {
    ...
    async listOperatorCapacities(channel, tenantId) {
        return this.routingRepository.listOperatorCapacities({
            ...(channel ? { channel } : {}),
            tenantId
        });
    }
    async operatorHasChannelAccess(operator, channel, tenantId) {
    ...
    operatorCanAccessChannel(operator, channel, memberships) {
        if (!channel) {
            return true;
        }
    ...
    async resolveRoutingPolicy(channel, tenantId) {
    ...
    async hydrateCanonicalRoutingState(tenantId, conversationId) {
        if (!this.canonicalWorkload || !this.canonicalConversations)
            return undefined;
    ...
    async persistManualTransition(input) {
        const current = this.routingRepository.readState();
        this.conversations = this.conversations.map((conversation) => conversation.id === input.conversation.id && conversation.tenantId === input.tenantId
    ...
    async persistBatchTransition(input) {
        const current = this.routingRepository.readState();
        const state = {
            ...current,
            conversations: this.conversations,
...
function auditEvent(scope, action, reason) {
    return {
...
function tenantContextRequiredEnvelope(operation) {
...
function normalizeAssignmentAction(action) {
    const value = String(action ?? "assign").trim();
    return supportedAssignmentActions.has(value) ? value : null;
}
function normalizeRescueOutcome(outcome) {
    const value = String(outcome ?? "saved").trim();
    return supportedRescueOutcomes.has(value) ? value : null;
}
function findCapacityForOperator(capacities, operatorId, selectedChannel) {
    return capacities.find((capacity) => capacity.operatorId === operatorId && (!selectedChannel || capacity.channel === selectedChannel));
}
function findRoutingPolicyRule(rules, selectedChannel) {
    return selectedChannel
        ? rules.find((rule) => rule.channel === selectedChannel) ?? rules.find((rule) => rule.channel === "*")
        : rules.find((rule) => rule.channel === "*");
}
function hasMembershipChannelAccess(memberships, operatorId, selectedChannel) {
    return memberships.some((membership) => membership.operatorId === operatorId
        && membership.active
        && membership.role !== "observer"
        && (!selectedChannel || membership.queueId === selectedChannel));
}
function findMembershipForOperator(memberships, operatorId, selectedChannel) {
    return memberships.find((membership) => membership.operatorId === operatorId
        && membership.active
        && (!selectedChannel || membership.queueId === selectedChannel));
}
...
function operatorProjection(operator, selectedChannel, capacity, channelAccessGranted = false) {
...
function queueProjection(queue) {
    return {
        ...clone(queue),
        name: typeof queue.name === "string"
            ? queue.name
            : queue.channel
    };
}
function routingAnalyticsProjection(rows, channel, tenantId) {
...
function requiredTenantId(context = {}) {
    const tenantId = String(context.tenantId ?? "").trim();
    return tenantId || null;
}
function normalizeSelectedQueues(selectedQueues) {
    return Array.from(new Set((selectedQueues ?? []).map((queue) => String(queue).trim()).filter(Boolean)));
}
function redistributionIdFromKey(idempotencyKey) {
    return `routing_redist_${sanitizeIdempotencyKey(idempotencyKey)}`;
}
function redistributionJobIdFromKey(idempotencyKey) {
...
function compareRedistributionConversations(left, right) {
...
function redistributionRiskRank(slaTone) {
    if (slaTone === "danger") {
        return 0;
    }
    if (slaTone === "warn") {
...
function realtimeEvent(eventName, resourceId, tenantId, data) {
    return {
...
function hasUnknownCanonicalPresence(operator) {
...
function withOperatorPresence(operator, presence) {
    if (!presence) {
        return operator;
    }
    return {
...
function operatorAcceptsAutoAssignment(operator) {
...
function operatorAcceptsManualAssignment(operator) {
    if (operator.presenceSource === "operator_presence") {
        return presenceAcceptsManualAssignment(operator.status);
    }
    return operator.status === "online" || hasUnknownCanonicalPresence(operator);
}
function presenceQualityLabel(operators) {
...
function replaceTenantRecords(current, canonical, tenantId) {
    return [...current.filter((record) => record.tenantId !== tenantId), ...canonical];
}
function reportEvent(eventName, outcome) {
    return {
...
function routingTraceId(operation) {
