...
export async function prepareVkCallback(input) {
    ...
    if (!confirmationCode)
        throw new Error("vk_confirmation_code_unavailable");
    return { confirmationCode };
}
/** Creates a VK Callback API server and enables new community messages. */
export async function subscribeVkCallback(input) {
    const secret = String(input.secret ?? "").trim();
    if (!/^[A-Za-z0-9]{1,24}$/.test(secret))
        throw new Error("vk_callback_secret_invalid");
    ...
    if (!Number.isInteger(serverId) || serverId <= 0)
        throw new Error("vk_callback_server_id_invalid");
...
async function callVk(method, parameters, input) {
    const groupId = String(input.groupId ?? "").trim();
    const token = String(input.accessToken ?? "").trim();
    if (!groupId)
        throw new Error("vk_group_id_required");
    if (!token)
...
function validateWebhookUrl(input) {
    ...
    try {
        ...
        throw new Error("vk_webhook_url_invalid");
    }
    if (url.protocol !== "https:" || url.port)
        throw new Error("vk_webhook_url_must_use_https_443");
    return url.toString();
}
function safeError(input) {
    return String(input ?? "").replace(/\s+/g, " ").slice(0, 200) || "unknown";
}
//# sourceMappingURL=vk-callback.js.map