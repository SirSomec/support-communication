const TENANT_OWNED_REPOSITORY_METHOD_CATALOG = [
...
export function createIdentityRepositoryTenantIsolationChecks(repository, tenantId) {
    return [
        {
...
export function createConversationRepositoryTenantIsolationChecks(repository, tenantId) {
    return [
        {
            expectedTenantId: tenantId,
...
export function createWorkspaceRepositoryTenantIsolationChecks(repository, input) {
    return [
        {
...
export function createBillingRepositoryTenantIsolationChecks(repository, tenantId) {
    return [
        {
            expectedTenantId: tenantId,
...
export function createRoutingRepositoryTenantIsolationChecks(repository, tenantId) {
    return [
        {
            expectedTenantId: tenantId,
...
export function createQualityRepositoryTenantIsolationChecks(repository, tenantId) {
    const checks = [
        {
            expectedTenantId: tenantId,
...
export async function verifyTenantIsolationChecks(checks) {
    const failures = [];
    if (checks.length === 0) {
        return {
            checked: 0,
...
function tenantScopedRecordId(row) {
...
function catalogEntry(service, method, expectedTenantSource) {
    return {
        boundary: "repository",
...
function resolveRecordId(row, index, selector) {
    if (selector) {
        return selector(row);
    }
...
function resolveTenantId(row, selector) {
    if (selector) {
        return selector(row);
    }
