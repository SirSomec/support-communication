...
import { canTransitionKnowledgeSourceStatus, deriveKnowledgeSourceReadiness } from "./knowledge-source.types.js";
...
export const knowledgeSourceBulkActions = ["archive", "delete", "disable", "enable"];
export class KnowledgeSourcesService {
    ...
    constructor(repository = KnowledgeSourceRepository.default(), workspaceRepository = WorkspaceRepository.default(), options = {}, policyRepository = UrlSourcePolicyRepository.default(), identityRepository = IdentityRepository.default(), automationRepository = AutomationRepository.default()) {
     ...
     */
    async unbindSourceEverywhere(tenantId, sourceId) {
        const state = await this.automationRepository.readStateAsync();
        const affected = [];
        for (const scenario of state.botScenarios) {
            if (scenario.tenantId !== tenantId || scenario.status === "archived")
            ...
            if (!bindingsChanged && !draftChanged)
                continue;
    ...
    async update(tenantId, sourceId, input) {
        ...
        if (!current)
            return invalid("updateKnowledgeSource", tenantId, "knowledge_source_not_found", "Источник знаний не найден.");
        const title = String(input.title ?? "").trim();
        if (!title)
            return invalid("updateKnowledgeSource", tenantId, "knowledge_source_title_required", "Укажите название источника.");
    ...
    async enable(tenantId, sourceId) {
        const current = await this.repository.find(tenantId, sourceId);
        if (!current)
            return invalid("enableKnowledgeSource", tenantId, "knowledge_source_not_found", "Источник знаний не найден.");
        if (current.status !== "disabled")
            return invalid("enableKnowledgeSource", tenantId, "knowledge_source_not_disabled", "Включить можно только отключённый источник.");
        ...
        const wasReady = current.kind === "mcp"
            ...
            || hasIndexedContent;
        const status = wasReady ? "ready" : "draft";
    ...
    async archive(tenantId, sourceId) {
        const current = await this.repository.find(tenantId, sourceId);
        if (!current)
            return invalid("archiveKnowledgeSource", tenantId, "knowledge_source_not_found", "Источник знаний не найден.");
    ...
    async remove(tenantId, sourceId) {
        const current = await this.repository.find(tenantId, sourceId);
        if (!current)
            return invalid("removeKnowledgeSource", tenantId, "knowledge_source_not_found", "Источник знаний не найден.");
        if (current.status !== "archived")
            return invalid("removeKnowledgeSource", tenantId, "knowledge_source_not_archived", "Сначала переместите источник в архив.");
    ...
    async preview(tenantId, sourceId) {
        const current = await this.repository.find(tenantId, sourceId);
        if (!current)
            return invalid("previewKnowledgeSource", tenantId, "knowledge_source_not_found", "Источник знаний не найден.");
        const chunks = Array.isArray(current.metadata.chunks) ? current.metadata.chunks : [];
    ...
    async create(tenantId, input) {
        ...
        const title = String(input.title ?? "").trim();
        if (!tenantId || !title || (kind !== "document" && kind !== "url" && kind !== "mcp"))
            return invalid("createKnowledgeSource", tenantId, "knowledge_source_invalid", "Choose a source type and provide a title.");
        const now = new Date().toISOString();
        ...
        if (kind === "document" && !sourceRef && sourceConfig.upload === true) {
            ...
            if (!sourceRef)
                return invalid("createKnowledgeSource", tenantId, "knowledge_article_required", "Choose a published knowledge article.");
            const article = await this.workspaceRepository.findKnowledgeArticle(sourceRef, { tenantId });
            if (!article || article.status !== "published")
                return invalid("createKnowledgeSource", tenantId, "knowledge_article_not_ready", "The selected article must be published before it can answer clients.");
            const prepared = preparePublishedArticle(article);
            if (!prepared)
                return invalid("createKnowledgeSource", tenantId, "knowledge_article_not_ready", "The selected article version must be published before it can answer clients.");
            status = "ready";
        ...
        if (kind === "url") {
            ...
            if (!validated.ok)
                return invalid("createKnowledgeSource", tenantId, validated.code, "This URL cannot be used as a knowledge source.");
        ...
        if (kind === "mcp") {
            ...
            if (!connectorId || !tool)
                return invalid("createKnowledgeSource", tenantId, "mcp_connector_required", "Выберите одобренный read-only MCP-коннектор и его инструмент.");
        ...
        const data = { source };
        if (kind === "url")
            data.auditEvent = await this.recordUrlAudit("knowledge_source.url.create", tenantId, source.id, "created");
        return envelope("createKnowledgeSource", tenantId, data);
    }
    async disable(tenantId, sourceId) {
        const current = await this.repository.find(tenantId, sourceId);
        if (!current)
            return invalid("disableKnowledgeSource", tenantId, "knowledge_source_not_found", "Knowledge source was not found.");
        ...
        const data = { source };
        if (current.kind === "url")
            data.auditEvent = await this.recordUrlAudit("knowledge_source.url.disable", tenantId, source.id, "disabled");
        return envelope("disableKnowledgeSource", tenantId, data);
    }
    async refreshUrl(tenantId, sourceId) {
        const current = await this.repository.find(tenantId, sourceId);
        if (!current || current.kind !== "url")
            return invalid("refreshKnowledgeSourceUrl", tenantId, "knowledge_source_not_found", "URL source was not found.");
        ...
        if (!validated.ok)
            return invalid("refreshKnowledgeSourceUrl", tenantId, validated.code, "This URL cannot be refreshed safely.");
        ...
        try {
            ...
            await assertPublicResolution(validated.hostname, this.options.resolveHostname);
            if (connectedPeerAddress && !isPublicAddress(connectedPeerAddress))
                throw new Error("url_source_peer_forbidden");
            const contentType = response.headers.get("content-type")?.toLowerCase() ?? "";
            const length = Number(response.headers.get("content-length") ?? 0);
            if (!response.ok || (!contentType.includes("text/html") && !contentType.includes("text/plain")) || !Number.isFinite(length) || length > URL_SOURCE_MAX_BYTES)
                throw new Error("url_source_fetch_rejected");
            ...
            if (!extractedText)
                throw new Error("url_source_content_empty");
     ...
     */
    async applyBulk(tenantId, action, input) {
        ...
        const sourceIds = Array.isArray(input.sourceIds)
            ...
            : [];
        if (!sourceIds.length)
            return invalid(operation, tenantId, "knowledge_bulk_request_invalid", "Укажите хотя бы один источник.");
        const affected = [];
        const skipped = [];
        for (const sourceId of sourceIds) {
            const outcome = await this.applyBulkOne(tenantId, action, sourceId);
            if (outcome.source)
                affected.push(outcome.source);
            else
                skipped.push({ code: outcome.code ?? "knowledge_source_transition_invalid", sourceId });
        }
        return envelope(operation, tenantId, { action, affected, skipped });
    }
    async applyBulkOne(tenantId, action, sourceId) {
        const current = await this.repository.find(tenantId, sourceId);
        if (!current)
            return { code: "knowledge_source_not_found" };
        if (action === "disable") {
            if (current.status === "disabled")
                return { code: "knowledge_source_already_disabled" };
            return unwrap(await this.disable(tenantId, sourceId));
        }
        if (action === "enable") {
            if (current.status !== "disabled")
                return { code: "knowledge_source_not_disabled" };
            return unwrap(await this.enable(tenantId, sourceId));
        }
        if (action === "archive") {
            if (current.status === "archived")
                return { code: "knowledge_source_already_archived" };
    ...
    async refreshDueUrls(now = new Date()) {
        let failed = 0;
        let refreshed = 0;
    ...
    async refreshDocument(tenantId, sourceId) {
        const current = await this.repository.find(tenantId, sourceId);
        if (!current || current.kind !== "document")
            return invalid("refreshKnowledgeSourceDocument", tenantId, "knowledge_source_not_found", "Document source was not found.");
    ...
    async recordUrlAudit(action, tenantId, sourceId, result) {
    ...
    async enqueueAttachmentIngestion(tenantId, sourceId, input) {
        const current = await this.repository.find(tenantId, sourceId);
        if (!current || current.kind !== "document")
            return invalid("enqueueKnowledgeAttachmentIngestion", tenantId, "knowledge_source_not_found", "Document source was not found.");
        const fileId = String(input.fileId ?? "").trim();
        const idempotencyKey = String(input.idempotencyKey ?? "").trim();
        if (!fileId || !idempotencyKey)
            return invalid("enqueueKnowledgeAttachmentIngestion", tenantId, "knowledge_ingestion_request_invalid", "A file and idempotency key are required.");
        const file = await this.workspaceRepository.findFile(fileId, { tenantId });
        if (!file || file.storageState !== "uploaded" || file.scanVerdict !== "clean" || !["clean", "scan_clean"].includes(file.scanState))
            return invalid("enqueueKnowledgeAttachmentIngestion", tenantId, "knowledge_attachment_scan_required", "Only uploaded and scan-clean attachments can be indexed.");
        const fingerprint = `${sourceId}:${fileId}`;
        const existing = await this.repository.findIngestionJob(tenantId, idempotencyKey);
        if (existing) {
            if (existing.fingerprint !== fingerprint)
                return invalid("enqueueKnowledgeAttachmentIngestion", tenantId, "idempotency_key_reused", "This idempotency key belongs to a different ingestion request.");
...
function preparePublishedArticle(article) {
...
function stripHtml(value) { return value.replace(/<script[\s\S]*?<\/script>|<style[\s\S]*?<\/style>/gi, " ").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim(); }
export async function assertPublicResolution(hostname, resolver) {
    const addresses = await (resolver ? resolver(hostname) : lookup(hostname, { all: true }));
    if (!addresses.length || addresses.some(({ address }) => !isPublicAddress(address)))
        throw new Error("url_source_dns_forbidden");
}
function isPublicAddress(address) {
    const host = address.includes(":") ? `[${address}]` : address;
    return validateUrlKnowledgeSourceConfig({ url: `https://${host}/` }).ok;
}
async function defaultUrlTransport(url, init) { return fetch(url, init); }
...
async function readBoundedText(response, maxBytes) {
    if (!response.body) {
        ...
        if (Buffer.byteLength(text, "utf8") > maxBytes)
            throw new Error("url_source_response_too_large");
        return text;
    }
...
function validationOptions(policy) { return policy.allowedHosts === null ? {} : { allowedHosts: policy.allowedHosts }; }
function validAllowlistHost(value) {
    const host = String(value ?? "").trim().toLowerCase().replace(/\.+$/, "");
    const validated = validateUrlKnowledgeSourceConfig({ url: `https://${host}/` });
    return validated.ok && validated.hostname === host;
}
function dueForRefresh(source, now) {
    const next = Date.parse(String(source.metadata.nextRefreshAt ?? ""));
    return !Number.isFinite(next) || next <= now.getTime();
}
function unwrap(result) {
    if (result.status !== "ok")
        return { code: result.error?.code ?? "knowledge_source_transition_invalid" };
    return { source: result.data.source };
}
function envelope(operation, tenantId, data) {
    return createEnvelope({ data, meta: { tenantId }, operation, service: SERVICE, traceId: `trc_${operation}_${randomUUID()}` });
}
function invalid(operation, tenantId, code, message, data = {}) {
...
//# sourceMappingURL=knowledge-sources.service.js.map