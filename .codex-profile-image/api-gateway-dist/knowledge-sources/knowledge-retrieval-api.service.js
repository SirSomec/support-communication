...
export class KnowledgeRetrievalApiService {
    ...
    retrieval = new KnowledgeRetrievalService(undefined, undefined, undefined, undefined, new LlmKnowledgeSearchService(), undefined, new SemanticKnowledgeSearchService()), automation = AutomationRepository.default()) {
        this.retrieval = retrieval;
        this.automation = automation;
    }
    async retrieveScenario(input) {
        const query = String(input.query ?? "").trim();
        if (!query)
            return result("invalid", input.tenantId, { passages: [], tokenBudget: 0, tokensUsed: 0 }, "knowledge_retrieval_query_required");
        const mode = input.mode === "llm" ? "llm" : input.mode === "semantic" ? "semantic" : "lexical";
        ...
        if (explicitSourceIds.length) {
            return result("ok", input.tenantId, (await this.retrieval.retrieve({
                mode,
                query,
        ...
        if (!scenario || scenario.tenantId !== input.tenantId || scenario.status === "archived")
            return result("invalid", input.tenantId, { passages: [], tokenBudget: 0, tokensUsed: 0 }, "knowledge_retrieval_scenario_not_found");
        return result("ok", input.tenantId, (await this.retrieval.retrieve({
            mode,
            query,
...
function result(status, tenantId, data, code) { return createEnvelope({ service: "knowledgeRetrievalService", operation: "retrieveScenarioKnowledge", traceId: `trc_knowledge_retrieval_${Date.now()}`, status, meta: { apiVersion: "v1", generatedAt: new Date().toISOString(), tenantId }, data, error: code ? { code, message: code } : null }); }
//# sourceMappingURL=knowledge-retrieval-api.service.js.map