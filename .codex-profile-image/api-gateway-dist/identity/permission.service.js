...
export class PermissionService {
    ...
    async validatePermission({ action, actorId = null, actorRole, resource, roleMode, tenantId }) {
...
function resolveSectionsForRole(role) {
    if (role.actions.includes("*")) {
        return ALL_SECTIONS;
    }
    return [...new Set(role.actions
            .map((action) => ACTION_TO_SECTION[action])
            .filter(Boolean))];
}
function resolvePermissionRole(roleMode, permissionRoles) {
    const value = String(roleMode ?? "").trim().toLowerCase();
    const normalizedValue = value === "senior operator" ? "senior" : value;
    return permissionRoles.find((role) => role.key.toLowerCase() === normalizedValue ||
        role.aliases.some((alias) => alias.toLowerCase() === normalizedValue));
}
function resolveGrant({ action, grants, resource, tenantId }) {
    const matching = grants.filter((grant) => ((grant.action === "*" || grant.action === action)
