...
import { createDisabledRealtimeFanoutAdapter, createRealtimeFanoutAdapterFromEnv } from "../conversation/realtime.fanout.js";
...
export class NotificationService {
    ...
    static configureRealtimeFanoutFromEnv(source = process.env) {
        defaultRealtimeFanout = createRealtimeFanoutAdapterFromEnv(source);
    }
    async fetchNotifications(filters = {}, context = {}) {
    ...
    async markNotificationsRead(payload, context = {}) {
        const tenantId = context.tenantId;
        if (!tenantId) {
    ...
    async fetchNotificationPreferences(context = {}) {
        const tenantId = context.tenantId;
        if (!tenantId) {
    ...
    async fetchBrowserPushPublicKey(context = {}) {
        const tenantId = context.tenantId;
        if (!tenantId) {
        ...
        if (!publicKey) {
            return invalidEnvelope("fetchBrowserPushPublicKey", "browser_push_public_key_required", "Browser push requires a configured VAPID public key.", { tenantId });
        }
        return createEnvelope({
            service: NOTIFICATION_SERVICE,
    ...
    async createBrowserPushSubscription(payload, context = {}) {
        const tenantId = context.tenantId;
        if (!tenantId) {
        ...
        if (!resolveBrowserPushPublicKey(process.env)) {
            return invalidEnvelope("createBrowserPushSubscription", "browser_push_public_key_required", "Browser push requires a configured VAPID public key.", { tenantId });
        }
        const normalized = normalizeBrowserPushSubscriptionPayload(payload);
        if (!normalized.ok) {
            return invalidEnvelope("createBrowserPushSubscription", normalized.code, normalized.message, { fields: normalized.fields, tenantId });
        }
    ...
    async deleteBrowserPushSubscription(subscriptionId, context = {}) {
        const tenantId = context.tenantId;
        if (!tenantId) {
        ...
        if (!normalizedSubscriptionId) {
            return invalidEnvelope("deleteBrowserPushSubscription", "browser_push_subscription_id_required", "Browser push subscription id is required.", { tenantId });
        }
        const now = new Date().toISOString();
        ...
        if (!revoked) {
            return invalidEnvelope("deleteBrowserPushSubscription", "browser_push_subscription_not_found", "Browser push subscription was not found for the tenant user.", { subscriptionId: normalizedSubscriptionId, tenantId });
        }
        const current = await this.notificationRepository.getNotificationPreferencesAsync({
            tenantId,
            userId: context.userId
    ...
    async updateNotificationPreferences(payload, context = {}) {
        const tenantId = context.tenantId;
        if (!tenantId) {
        ...
        if (!externalChannelValidation.ok) {
            return invalidEnvelope("updateNotificationPreferences", "notification_external_channel_unavailable", "Notification delivery channel must be an active connection owned by the tenant.", {
        ...
        if (nextBrowserPushEnabled && activeBrowserPushSubscription?.status !== "active") {
            return invalidEnvelope("updateNotificationPreferences", "browser_push_subscription_required", "Browser push can be enabled only after a deliverable push subscription is stored.", { tenantId });
        }
    ...
    async sendCriticalAlertTest(payload, context = {}) {
        const tenantId = context.tenantId;
        if (!tenantId) {
        ...
        if (!externalChannelValidation.ok) {
            return invalidEnvelope("sendCriticalAlertTest", "notification_external_channel_unavailable", "Notification delivery channel must be an active connection owned by the tenant.", {
                unavailableChannelIds: externalChannelValidation.unavailableChannelIds,
                tenantId
            });
        }
        const deliveryResults = externalChannelValidation.channels.map((channel) => ({
        ...
        if (payload?.includeBrowserPush === true) {
            ...
            if (!preferences.browserPushEnabled || browserPushSubscription?.status !== "active") {
                return invalidEnvelope("sendCriticalAlertTest", "browser_push_subscription_required", "Browser push test requires a stored deliverable push subscription.", { tenantId });
            }
        }
        if (!deliveryResults.length && !browserPushSubscription) {
            return invalidEnvelope("sendCriticalAlertTest", "notification_delivery_channel_required", "At least one active notification delivery channel or push subscription is required.", { tenantId });
        }
        ...
        const notification = created.data.notification;
        const now = new Date().toISOString();
    ...
    async publishRealtimeEvent(eventName, notification, extra) {
        const event = {
            data: {
                notification: toNotificationView(notification),
                ...extra
    ...
    async resolveDeliverableExternalChannels(tenantId, channelIds) {
        const unavailableChannelIds = [];
        const channels = [];
...
function toNotificationPreferencesView(preferences) {
    return {
...
function toBrowserPushSubscriptionView(subscription) {
    return {
...
function toNotificationPreferenceAuditView(event) {
    return {
        action: event.action,
        at: event.at,
        id: event.id,
...
function notificationTraceId(operation) {
...
function nullablePayloadString(value) {
    const normalized = String(value ?? "").trim();
    return normalized ? normalized : null;
}
function resolveBrowserPushPublicKey(source) {
...
function normalizeBrowserPushSubscriptionPayload(payload) {
    if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
        return {
...
function isDeliverablePushEndpoint(value) {
    try {
...
function hashBrowserPushEndpoint(endpoint) {
