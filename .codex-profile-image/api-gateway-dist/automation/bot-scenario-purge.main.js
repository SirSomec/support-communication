...
export async function runBotScenarioPurgeFromEnv(source = process.env, argv = process.argv) {
    ...
    if (argv.includes("--once") || source.BOT_SCENARIO_PURGE_ONCE === "true") {
        console.log(JSON.stringify({ result: first, service: "bot-scenario-purge-worker" }));
        return;
    }
    let running = false;
    setInterval(() => {
...
//# sourceMappingURL=bot-scenario-purge.main.js.map