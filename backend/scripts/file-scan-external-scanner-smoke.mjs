...
if (process.env.FILE_SCAN_EXTERNAL_SCANNER_SMOKE_ENABLED !== "true") {
  process.stdout.write("file scan external scanner smoke skipped {\"reason\":\"FILE_SCAN_EXTERNAL_SCANNER_SMOKE_ENABLED is not true\"}\n");
  process.exit(0);
}

const backendRoot = fileURLToPath(new URL("..", import.meta.url));
const smokePrefix = "file_scan_external_scanner_smoke";
...
const scannerUrl = requireConfigured(process.env.OUTBOX_SCANNER_URL, "OUTBOX_SCANNER_URL");
const expectedScanner = stringValue(process.env.FILE_SCAN_EXTERNAL_SCANNER_EXPECTED_SCANNER);
const signedFileUrl = stringValue(process.env.FILE_SCAN_EXTERNAL_SCANNER_SIGNED_FILE_URL);
const signedFileExpiresAt = stringValue(process.env.FILE_SCAN_EXTERNAL_SCANNER_SIGNED_FILE_EXPIRES_AT)
...
const smoke = {
  ...
  fileId: `file_${runId}`,
  fileName: "file-scan-external-scanner-smoke.txt",
  idempotencyKey: `idempotency_${runId}`,
  objectKey: `smoke/${runId}/file-scan-external-scanner-smoke.txt`,
...
try {
  ...
  if (result.scanned !== 1 || result.published !== 1 || result.failed !== 0) {
    throw new Error(`file_scan_external_scanner_smoke_unexpected_result:${JSON.stringify(result)}`);
  }

  const persisted = await assertPersistedScanResult(client, smoke);
  process.stdout.write(`file scan external scanner smoke passed ${JSON.stringify({
    fileId: smoke.fileId,
    outboxEventId: smoke.outboxEventId,
    result,
    scanState: persisted.scanState,
    scanVerdict: persisted.scanVerdict,
    scanner: persisted.scanner
  })}\n`);
} finally {
  if (process.env.FILE_SCAN_EXTERNAL_SCANNER_SMOKE_KEEP_DATA !== "true") {
...
async function loginServiceAdmin(baseUrl) {
  const email = stringValue(process.env.FILE_SCAN_EXTERNAL_SCANNER_SMOKE_SERVICE_ADMIN_EMAIL)
    || stringValue(process.env.FILE_SCAN_API_CALLBACK_SMOKE_SERVICE_ADMIN_EMAIL)
    || "service-admin@example.com";
  const password = stringValue(process.env.FILE_SCAN_EXTERNAL_SCANNER_SMOKE_SERVICE_ADMIN_PASSWORD)
    || stringValue(process.env.FILE_SCAN_API_CALLBACK_SMOKE_SERVICE_ADMIN_PASSWORD)
    || "correct-password";
  ...
  if (!mfaChallengeId) {
    throw new Error("file_scan_external_scanner_smoke_mfa_challenge_missing");
  }
  const otp = stringValue(process.env.FILE_SCAN_EXTERNAL_SCANNER_SMOKE_SERVICE_ADMIN_OTP)
    || stringValue(process.env.FILE_SCAN_API_CALLBACK_SMOKE_SERVICE_ADMIN_OTP)
  ...
  if (!accessToken) {
    throw new Error("file_scan_external_scanner_smoke_access_token_missing");
  }
  return accessToken;
}
...
async function postJson(url, body) {
  ...
  if (!response.ok || payload.status !== "ok") {
    throw new Error(`file_scan_external_scanner_smoke_http_failed:${response.status}:${JSON.stringify(payload)}`);
  }
  return payload;
}
  ...
  await prisma.$transaction(async (transaction) => {
    await transaction.workspaceFile.create({
      data: {
        ...
        checksum: "sha256:file-scan-external-scanner-smoke",
        fileId: input.fileId,
    ...
    await transaction.conversationOutboundDescriptor.create({
      data: {
        ...
        payload: {
          ...
          mimeType: "text/plain",
          queue: input.queue,
          ...(input.signedFile ? { signedFile: input.signedFile } : {}),
...
async function runWorkerOnce(baseUrl, accessToken, input) {
  const env = {
    ...
    OUTBOX_SCANNER_BEARER_TOKEN: stringValue(process.env.OUTBOX_SCANNER_BEARER_TOKEN),
    OUTBOX_SCANNER_ENABLED: "true",
    OUTBOX_SCANNER_PROVIDER_MODE: "http",
    OUTBOX_SCANNER_URL: scannerUrl
  };
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once("close", (code) => {
      if (code !== 0) {
        reject(new Error(`file_scan_external_scanner_worker_failed:${code ?? 1}:${stderr || stdout}`));
        return;
      }
...
function parseWorkerRunResult(stdout) {
  ...
  if (!workerRecord) {
    throw new Error("file_scan_external_scanner_worker_result_not_found");
  }
...
async function assertPersistedScanResult(prisma, input) {
  ...
  if (!event || event.status !== "published" || !event.publishedAt) {
    throw new Error(`file_scan_external_scanner_outbox_not_published:${event?.status ?? "missing"}`);
  }

  if (!file || file.scanVerdict !== "clean" || file.scanState !== "scan_clean") {
    throw new Error(`file_scan_external_scanner_scan_not_persisted:${file?.scanVerdict ?? "missing"}:${file?.scanState ?? "missing"}`);
  }

  if (!file.scanner || file.scanner === "local-deterministic-scanner") {
    throw new Error(`file_scan_external_scanner_unexpected_scanner:${file?.scanner ?? "missing"}`);
  }

  if (expectedScanner && file.scanner !== expectedScanner) {
    throw new Error(`file_scan_external_scanner_name_mismatch:${file.scanner}:${expectedScanner}`);
  }

  if (!idempotency || idempotency.fileId !== input.fileId) {
    throw new Error("file_scan_external_scanner_idempotency_not_persisted");
  }
