...
- Composer поддерживает ответ клиенту, внутренний комментарий, вставку шаблонов, имитацию вложения, AI-подсказку и сохранение текста как шаблона.
...
| Тематика в любое время чата | Реализована в шапке и правой карточке | Справочник тематик, многоуровневость, поиск, архивность, права редактирования |
...
| SDK-подключение | Есть SDK-консоль, события и snippet | Playground с payload preview, ключи окружений, тестовый стенд, changelog |
| Telegram, MAX, VK | Есть каналы, статусы, health, настройки | Детальные страницы каналов, несколько подключений, журнал ошибок, тест приема/отправки |
...
| Роли сотрудник/старший/администратор | Есть role switcher и матрица | Применить ограничения ко всем разделам, не только настройкам |
...
| Телефон, устройство, точка входа | Реализовано в карточке клиента и SDK/visitor данных | Device timeline и полная детальная страница клиента |
...
| Клиентские профили и объединение | Частично: список и карточка | Детальная страница, merge/unmerge дублей, чувствительные поля по правам |
| Поиск и фильтры | Частично: поиск диалогов, клиентов, шаблонов, отчеты | Полный набор фильтров по статусу, дате, оператору, тематике, каналу, внутренним комментариям |
| Вложения | Частично: UI-кнопка и имитация вложения | Upload queue, preview, ошибки размера/типа, канальные ограничения |
...
| Audit | Есть фильтр в чате, audit events, export audit | Единый audit log с фильтрами и деталкой события |
...
- Добавить расширенные фильтры: канал, тематика, оператор, статус, дата, телефон, клиентский ID, внутренний комментарий.
- Добавить сортировки: последнее сообщение, SLA, дата создания, приоритет, статус.
- Добавить полноценные статусы обращения: новое, в очереди, назначено, в работе, ожидает клиента, ожидает оператора, передано, на паузе, закрыто, переоткрыто.
- Добавить вложения: upload queue, preview, ошибки размера и типа, канальные ограничения.
- Добавить warning о несохраненном черновике при смене диалога.
...
- Добавить audit timeline для смены статусов, назначения, передачи, тематики и закрытия.
...
- Применить role mode не только в `Настройки`, но и в `Диалоги`, `Панель`, `Клиенты`, `Шаблоны`, `Отчеты`, `Визиты`, `Качество`, `Боты`.
...
1. Применить ролевые ограничения ко всем разделам, а не только к настройкам.
2. Добавить детальную страницу клиента и merge/unmerge дублей.
3. Добавить детальные страницы каналов с логами ошибок, тестом сообщения и raw IDs.
4. Добавить расширенные фильтры и сортировки очереди диалогов.
5. Довести вложения в composer до upload/preview/error UI.
6. Расширить отчеты chart-блоками, настройкой колонок и real export ready state.
7. Добавить inline AI-панель в чат с accept/edit/reject и audit.
8. Добавить rescue timer в сам чат и настройки rescue по каналу/очереди/роли.
9. Добавить visual builder proactive-правил с preview и A/B управлением.
10. Добавить canvas/flow-builder ботов с тестовым transcript preview.
11. Разнести `App.jsx`, `sections.jsx`, `data.js` по feature-модулям.
12. Добавить системные empty/loading/error states и smoke/e2e для критичных flows.
...
- [ ] Добавить расширенные фильтры и сортировки очереди диалогов.
- [ ] Добавить полноценные статусы обращения и audit timeline.
- [ ] Добавить upload/preview/error UI для вложений.
- [ ] Добавить warning о несохраненном черновике при смене диалога.
- [ ] Добавить детальную страницу клиента и merge/unmerge дублей.
- [ ] Добавить многоуровневый справочник тематик с поиском, архивностью и правами.
- [ ] Добавить детальные страницы каналов с логами, тестом сообщения и несколькими подключениями.
- [ ] Добавить SDK playground с raw payload preview.
- [ ] Применить role switcher и disabled states ко всем разделам.
