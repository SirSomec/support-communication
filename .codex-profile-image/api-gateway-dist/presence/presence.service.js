...
import { isOperatorPresenceStatus, OPERATOR_PRESENCE_STATUSES, presenceAcceptsAutoAssignment } from "./operator-presence.types.js";
...
let defaultRealtimeFanout = createDisabledRealtimeFanoutAdapter("presence_realtime_fanout_not_configured");
export class OperatorPresenceService {
    autoAssignQueuedConversations;
    ...
    async setMyPresence(payload, context = {}) {
     ...
     */
    async markMyPresenceUnavailableIfOnline(context = {}) {
    ...
    async fetchTeamPresence(filters = {}, context = {}) {
    ...
    async resolveOperatorName(tenantId, operatorId) {
        try {
            const users = await this.identityRepository.findTenantUsers(tenantId);
            return users.find((user) => user.id === operatorId)?.name ?? operatorId;
        }
        catch {
            return operatorId;
        }
    }
    async publishPresenceUpdate(input) {
        const event = {
            data: {
...
function requireOperatorScope(operation, context) {
    const tenantId = String(context.tenantId ?? "").trim();
...
function resolvePresenceRange(filters) {
    const now = new Date();
...
function summarizeIntervalSeconds(intervals, range) {
...
function errorEnvelope(operation, status, code, message, details) {
    return createEnvelope({
        service: PRESENCE_SERVICE,
        operation,
        status,
...
function presenceTraceId(operation) {
