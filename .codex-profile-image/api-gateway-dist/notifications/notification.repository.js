...
let defaultNotificationRepository = null;
export class NotificationRepository {
    ...
    static inMemory(seed = seedNotificationState()) {
    ...
    async listNotificationsAsync(filter = {}) {
        if (this.prismaClient) {
            if (!filter.tenantId) {
                return [];
            }
    ...
    findNotification(notificationId, tenantId) {
        if (this.prismaClient) {
            throw new Error("prisma_notifications_async_required");
        }
        return this.readState().notifications.find((notification) => notification.id === notificationId
            && (!tenantId || notification.tenantId === tenantId));
    }
    async findNotificationAsync(notificationId, tenantId) {
        if (this.prismaClient) {
    ...
    async saveNotificationAsync(record) {
        const persisted = normalizeNotification(record);
        if (this.prismaClient) {
    ...
    getNotificationPreferences(input) {
        ...
        const existing = this.readState().preferences?.find((preference) => preference.tenantId === tenantId
    ...
    async getNotificationPreferencesAsync(input) {
        const tenantId = requireString(input.tenantId);
        const userId = nullableString(input.userId);
        if (this.prismaClient) {
    ...
    saveNotificationPreferences(record) {
        if (this.prismaClient) {
            throw new Error("prisma_notification_preferences_async_required");
        }
        ...
        this.store.update((state) => {
            const normalized = normalizeState(state);
            const exists = normalized.preferences.some((preference) => preference.tenantId === persisted.tenantId
                && preference.userId === persisted.userId);
            return {
                ...normalized,
                preferences: exists
                    ? normalized.preferences.map((preference) => preference.tenantId === persisted.tenantId && preference.userId === persisted.userId ? persisted : preference)
                    : [...normalized.preferences, persisted]
            };
        });
        return clone(persisted);
    }
    async saveNotificationPreferencesAsync(record) {
        const persisted = normalizeNotificationPreferences(record);
        if (this.prismaClient) {
    ...
    saveBrowserPushSubscription(record) {
        if (this.prismaClient) {
    ...
    async saveBrowserPushSubscriptionAsync(record) {
        const persisted = normalizeBrowserPushSubscription(record);
        if (this.prismaClient) {
    ...
    findBrowserPushSubscription(input) {
        ...
        const userId = nullableString(input.userId);
        return clone(this.readState().browserPushSubscriptions.find((subscription) => subscription.id === subscriptionId
    ...
    saveNotificationDeliveryDescriptor(descriptor) {
        if (this.prismaClient) {
    ...
    async saveNotificationDeliveryDescriptorAsync(descriptor) {
        const persisted = normalizeNotificationDeliveryDescriptor(descriptor);
        if (this.prismaClient) {
    ...
    updateNotificationDeliveryDescriptor(descriptorId, update, expectedClaimedAt) {
        if (this.prismaClient) {
            throw new Error("prisma_notification_delivery_descriptors_async_required");
        }
        ...
        this.store.update((state) => {
            ...
            return {
                ...
                deliveryDescriptors: normalized.deliveryDescriptors.map((descriptor) => {
                    ...
                    if (expectedClaimedAt && (descriptor.status !== "processing"
    ...
    async updateNotificationDeliveryDescriptorAsync(descriptorId, update, expectedClaimedAt) {
        if (this.prismaClient) {
            const normalizedDescriptorId = requireString(descriptorId);
    ...
    recordPreferenceAuditEvent(event) {
        if (this.prismaClient) {
    ...
    async recordPreferenceAuditEventAsync(event) {
        const persisted = normalizePreferenceAuditEvent(event);
        if (this.prismaClient) {
...
function normalizeCategory(value) {
    ...
    return categories.has(value) ? value : "sla_risk";
}
function normalizeTone(value) {
    const tones = new Set(["danger", "info", "ok", "warn"]);
    return tones.has(value) ? value : "info";
}
function normalizeNotificationActionTarget(value) {
...
function defaultNotificationPreferences({ now = new Date().toISOString(), tenantId, userId }) {
    return {
...
function normalizeNotificationPreferences(record) {
    return {
...
function normalizeBrowserPushSubscription(record) {
...
function normalizeNotificationDeliveryDescriptor(descriptor) {
...
function normalizePreferenceAuditEvent(event) {
    return {
...
function notificationPreferenceId(tenantId, userId) {
    return `${tenantId}:${userId ?? "tenant"}`;
}
function toPrismaNotificationCreateInput(record) {
    return {
...
function toPrismaNotificationUpdateInput(create) {
    return {
...
function toPrismaNotificationPreferenceCreateInput(record) {
    return {
...
function toPrismaNotificationPreferenceUpdateInput(create) {
    return {
...
function toNotificationPreferencesRecord(row) {
...
function toPrismaBrowserPushSubscriptionCreateInput(record) {
    return {
        createdAt: new Date(record.createdAt),
...
function toPrismaBrowserPushSubscriptionUpdateInput(create) {
    return {
...
function toBrowserPushSubscriptionRecord(row) {
...
function toPrismaNotificationDeliveryDescriptorCreateInput(descriptor) {
...
function toPrismaNotificationDeliveryDescriptorUpdateInput(create) {
    return {
...
function toNotificationDeliveryDescriptor(row) {
...
function toPrismaNotificationPreferenceAuditEventCreateInput(event) {
    return {
        action: event.action,
...
function toPrismaNotificationPreferenceAuditEventUpdateInput(create) {
    return {
        action: create.action,
        at: create.at,
...
function toNotificationPreferenceAuditEvent(row) {
    if (!row.immutable) {
...
function assertCompletePrismaNotificationClient(client) {
