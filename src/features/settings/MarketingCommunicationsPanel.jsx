...
export function MarketingCommunicationsPanel({ onToast }) {
  ...
  const saveSettings = async (event) => {
    ...
    onToast?.(response.status === "ok" ? "Правила маркетинговых коммуникаций сохранены." : response.error?.message ?? "Не удалось сохранить настройки.");
    if (response.status === "ok") void load();
  };
  ...
  if (status === "loading") return <section className="marketing-settings-panel-card"><span>Загружаем настройки коммуникаций…</span></section>;
  if (status === "forbidden") return <section className="marketing-settings-panel-card marketing-settings-panel-empty"><ShieldCheck size={24} /><div><h2>Управление доступно владельцу</h2><p>Владелец организации может выдавать доступ к коммуникациям и управлять API-ключами.</p></div></section>;
  if (status === "error") return <section className="marketing-settings-panel-card marketing-settings-panel-empty"><div><h2>Настройки коммуникаций недоступны</h2><p>Не удалось загрузить данные. Повторите попытку.</p></div><button onClick={load} type="button"><RefreshCw size={16} /> Повторить</button></section>;
  ...
  return <section className="marketing-settings-global">
    <header><div><span className="settings-panel-kicker">Коммуникации</span><h2>Доступы и API</h2><p>Выдавайте доступ к маркетинговым коммуникациям и управляйте интеграционными ключами.</p></div><button onClick={load} type="button"><RefreshCw size={16} /> Обновить</button></header>
    ...
    <form className="marketing-settings-panel-card marketing-settings-rules" onSubmit={saveSettings}><div className="marketing-settings-panel-title"><ShieldCheck size={19} /><div><h3>Согласия и тихие часы</h3><p>Если согласия ещё нет, клиент сначала получит запрос. Любой следующий ответ в этом канале подтвердит согласие, после чего коммуникация продолжится автоматически.</p></div></div><div><label>Тихие часы с<select value={settingsDraft.quietHoursStart} onChange={(event) => setSettingsDraft((value) => ({ ...value, quietHoursStart: event.target.value }))}>{Array.from({ length: 24 }, (_, hour) => <option key={hour} value={hour}>{String(hour).padStart(2, "0")}:00</option>)}</select></label><label>до<select value={settingsDraft.quietHoursEnd} onChange={(event) => setSettingsDraft((value) => ({ ...value, quietHoursEnd: event.target.value }))}>{Array.from({ length: 24 }, (_, hour) => <option key={hour} value={hour}>{String(hour).padStart(2, "0")}:00</option>)}</select></label></div><label>Текст запроса согласия<textarea value={settingsDraft.consentText} onChange={(event) => setSettingsDraft((value) => ({ ...value, consentText: event.target.value }))} /></label><button className="primary-action" type="submit"><Check size={16} /> Сохранить правила</button></form>
  </section>;
}
