...
export class BillingService {
    billingRepository;
    constructor(billingRepository = BillingRepository.default()) {
    ...
    async previewTariffChange(payload) {
    ...
    async changeTenantTariff(payload) {
        const request = payload ?? {};
        const tenant = await this.findTenant(request.tenantId);
        const nextTariff = await this.findTariff(request.nextPlanId);
        if (!tenant) {
    ...
    async commitQuotaReservation(payload) {
        const request = payload ?? {};
        const idempotencyKey = request.idempotencyKey?.trim();
        if (!idempotencyKey) {
    ...
    async releaseQuotaReservation(payload) {
        const request = payload ?? {};
        const idempotencyKey = request.idempotencyKey?.trim();
        if (!idempotencyKey) {
    ...
    async fetchTenantQuotaSnapshot(tenantId) {
        ...
        return createEnvelope({
            ...
            data: {
                quotas: quotas.filter((metric) => Boolean(metric)),
                tariff: tariff ? clone(tariff) : null,
                tenant: tenantSummary(tenant)
            }
        });
    }
    async fetchTenantSubscription(tenantId) {
        const tenant = await this.findTenant(tenantId);
        if (!tenant) {
    ...
    async syncProviderBillingState(payload) {
        const request = payload ?? {};
        const tenant = await this.findTenant(request.tenantId);
        if (!tenant) {
        ...
        if (cached) {
            ...
            let updatedEvent;
            try {
                updatedEvent = await this.billingRepository.appendProviderSyncAuditEvent(cached.idempotencyKey, providerSyncAuditEvent(cached, "duplicate"));
            }
            catch {
    ...
    async validateTariffApproval(input) {
    ...
    async validatePaymentActionApproval(input) {
        if (!input.invoice) {
    ...
    async activeReservedAmount(tenantId, resource) {
    ...
    async recordQuotaLedgerDecision(input, request) {
        const idempotencyKey = request.idempotencyKey?.trim();
        if (!idempotencyKey) {
            return {
                kind: "error",
...
function billingTraceId(service, operation) {
    return getCurrentTraceId() ?? createRequestTraceId(service, operation);
}
function buildBillingAuditEvent({ action, reason, result, severity, target, tenantId, traceId }) {
    return {
        action,
        actor: "quota-service",
        actorName: "Quota Service",
...
function buildTariffPreview(tenant, currentTariff, nextTariff, reason) {
    const capacityCheck = {
...
function confirmationText(tenantId, planId) {
    return `CHANGE ${tenantId} TO ${planId}`;
}
function tariffApprovalSubjectId(tenantId, currentPlanId, nextPlanId) {
    return `${tenantId}:${currentPlanId}:${nextPlanId}`;
}
function paymentActionApprovalSubjectId(tenantId, invoiceId, eventType) {
    return `${tenantId}:${invoiceId}:${eventType}`;
}
function requiresPaymentActionApproval(provider, eventType) {
...
function errorEnvelope(service, operation, code, message, data) {
    return createEnvelope({
        service,
        operation,
        traceId: billingTraceId(service, operation),
...
function providerSyncAuditEvent(event, result) {
    return {
...
function quotaLedgerResponseData(entry, duplicate) {
    return {
...
function quotaReservationResponseData(reservation, duplicate, auditAction) {
...
function quotaMetric(tenant, tariff, resource, reserved = 0) {
    const metrics = {
...
function normalizeProviderEventType(eventType) {
    const value = String(eventType ?? "").trim();
    return value || undefined;
}
function normalizeSubscriptionSync(subscription, tenant, provider, now) {
    if (!subscription) {
        return undefined;
    }
...
function normalizeInvoiceSync(invoice, tenant, subscription, provider, now) {
    if (!invoice) {
        return undefined;
    }
...
function providerTenantChanges(tenant, subscription) {
    if (!subscription) {
        return {};
    }
    const changes = {};
...
function subscriptionFingerprintData(subscription) {
    return {
...
function invoiceFingerprintData(invoice) {
    return {
