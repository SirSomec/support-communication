...
import { createOpenAiCompatibleChatProvider, AiProviderError } from "./openai-compatible-chat.provider.js";
...
export class AiConnectionsService {
    ...
    identityRepository;
    testProviderFactory;
    constructor(repository = AiConnectionRepository.default(), environment = process.env, usage = AiUsageRepository.default(), identityRepository = IdentityRepository.default(), testProviderFactory = createOpenAiCompatibleChatProvider) {
        this.repository = repository;
        this.environment = environment;
        this.usage = usage;
        this.identityRepository = identityRepository;
        this.testProviderFactory = testProviderFactory;
    }
    async list(tenantId) {
    ...
    async create(tenantId, input) {
        ...
        if (violation)
            return invalid("createAiConnection", tenantId, violation);
        try {
    ...
    async update(tenantId, connectionId, input) {
        const existing = await this.repository.find(tenantId, connectionId);
        if (!existing)
            return notFound("updateAiConnection", tenantId, connectionId);
        const violation = validateWrite(input, false);
        if (violation)
            return invalid("updateAiConnection", tenantId, violation);
        try {
    ...
    async rotate(tenantId, connectionId, input) {
        if (!input.secret?.trim())
            return invalid("rotateAiConnection", tenantId, "API key is required.");
        const result = await this.update(tenantId, connectionId, { secret: input.secret });
        return { ...result, operation: "rotateAiConnection" };
    }
    async test(tenantId, connectionId) {
        const existing = await this.repository.find(tenantId, connectionId);
        if (!existing)
            return notFound("testAiConnection", tenantId, connectionId);
        const now = new Date().toISOString();
    ...
    async disable(tenantId, connectionId) {
        const existing = await this.repository.find(tenantId, connectionId);
        if (!existing)
            return notFound("disableAiConnection", tenantId, connectionId);
        const now = new Date().toISOString();
    ...
    async remove(tenantId, connectionId) {
        if (!(await this.repository.remove(tenantId, connectionId)))
            return notFound("deleteAiConnection", tenantId, connectionId);
    ...
    async recordAudit(action, tenantId, connectionId, result, traceId = `trc_${randomUUID()}`) {
...
function envelope(operation, tenantId, data, status = "ok", error = null, traceId = `trc_${operation}_${randomUUID()}`) {
    return createEnvelope({ data, error, meta: { tenantId }, operation, service: SERVICE, status, traceId });
}
function invalid(operation, tenantId, message) { return envelope(operation, tenantId, {}, "invalid", { code: "ai_connection_invalid", message }); }
function notFound(operation, tenantId, connectionId) { return envelope(operation, tenantId, {}, "invalid", { code: "ai_connection_not_found", message: `AI connection ${connectionId} was not found.` }); }
function publicConnection(connection) { const { secret: _secret, ...safe } = connection; return { ...safe, secretConfigured: true }; }
function normalizeBaseUrl(value) { const url = new URL(value.trim()); if (url.protocol !== "https:" || url.username || url.password)
    throw new Error("AI provider URL must use HTTPS."); return url.toString().replace(/\/$/, ""); }
function normalizedOptional(value) { const normalized = String(value ?? "").trim(); return normalized || null; }
function normalizeCapabilities(value) { const capabilities = Array.from(new Set(value ?? [])); if (!capabilities.includes("chat_completion"))
    capabilities.unshift("chat_completion"); return capabilities.filter((item) => item === "chat_completion" || item === "embeddings" || item === "retrieval"); }
/** BAI-870: заполненная модель роли автоматически даёт capability — инвариант держится без чекбоксов в UI. */
function deriveCapabilities(capabilities, models) {
    const derived = new Set(capabilities);
    if (models.retrievalModel)
        derived.add("retrieval");
    if (models.embeddingModel)
        derived.add("embeddings");
    return normalizeCapabilities([...derived]);
}
function normalizeLimits(value) { const source = value ?? {}; const limits = {}; for (const key of ["maxConcurrentRuns", "monthlyTokenBudget", "requestsPerMinute", "sandboxMonthlyTokenBudget"]) {
    const parsed = Number(source[key]);
    if (Number.isInteger(parsed) && parsed > 0)
        limits[key] = parsed;
} return limits; }
function validateWrite(input, creating) { if (creating && !input.secret?.trim())
    return "API key is required."; if (input.secret !== undefined && !input.secret.trim())
    return "API key must not be empty."; if (creating && (!input.baseUrl || !input.chatModel))
    return "Provider URL and chat model are required."; try {
    if (input.baseUrl)
        normalizeBaseUrl(input.baseUrl);
}
catch {
    return "Provider URL must be a valid HTTPS URL.";
} return null; }
function safeMessage(error) { if (error instanceof AiProviderError)
    return error.message; if (error instanceof SecretStoreError)
    return "Secret storage is unavailable."; return "AI connection could not be completed safely."; }
function logWriteFailure(operation, tenantId, error) {
...
function testDiagnostic(error) { return error instanceof AiProviderError ? error.code : error instanceof SecretStoreError ? "secret_storage_unavailable" : "provider_unavailable"; }
//# sourceMappingURL=ai-connections.service.js.map