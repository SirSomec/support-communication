...
const KNOWN_TRANSPORT_CODES = new Set(SMTP_TRANSPORT_ERROR_CODES);
 ...
 */
export function sendSmtpMail(config, mail) {
    return new Promise((resolve, reject) => {
        ...
        const run = async (socket, upgraded) => {
            activeSocket = socket;
...
export function composeMailMessage(config, mail) {
...
export function encodeMailHeaderText(value) {
...
export function smtpMailAddress(value, errorCode) {
...
export function smtpMailHost(value, errorCode) {
    const normalized = String(value ?? "").trim();
...
function encodeSmtpPlainAuth(auth) {
    return Buffer.from(`${SMTP_AUTH_SEPARATOR}${auth.username}${SMTP_AUTH_SEPARATOR}${auth.password}`, "utf8").toString("base64");
}
 ...
 */
function refineStepError(error, stepCode) {
