...
export async function runKnowledgeDocumentIngestionFromEnv(source = process.env, argv = process.argv) {
    ...
    if (argv.includes("--once") || source.KNOWLEDGE_DOCUMENT_INGESTION_ONCE === "true") {
        console.log(JSON.stringify({ result: first, service: "knowledge-document-ingestion-worker" }));
        return;
    }
    const interval = positive(source.KNOWLEDGE_DOCUMENT_INGESTION_INTERVAL_MS, 5_000);
    let running = false;
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve(process.argv[1]))
    void runKnowledgeDocumentIngestionFromEnv().catch((error) => { console.error(error); process.exitCode = 1; });
//# sourceMappingURL=document-ingestion.main.js.map