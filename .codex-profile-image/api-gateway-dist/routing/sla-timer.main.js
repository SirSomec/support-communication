...
import { applySlaTimerTransition, claimDueSlaTimerJobs, planSlaTimerTransition } from "./sla-timer.worker.js";
import { installRoutingWorkerShutdownHandlers, loadRoutingWorkerRuntimeConfig, positiveInteger, runRoutingWorkerRuntime } from "./routing-worker.runtime.js";
export async function executeSlaTimerWorkerOnce(repository, config, now = new Date()) {
    ...
    const summary = { applied: 0, claimed: jobs.length, deadLettered: 0, failed: 0, skipped: 0 };
...
export async function runSlaTimerWorkerFromEnv(source = process.env, argv = process.argv) {
...
async function completeSkippedJob(repository, job, now) {
...
async function recordOwnedSlaFailure(repository, claimedJob, error, config, now) {
...
function ownsActiveLease(job, leaseOwner, now) {
...
//# sourceMappingURL=sla-timer.main.js.map