...
import { makeEphemeralPlatformMutationIdempotencyKey, persistPlatformIncidentMutationAsync } from "../platform/platform-audit-outbox.js";
...
export class IncidentService {
    ...
    constructor(platformRepository = PlatformRepository.default()) {
        ...
        try {
            this.idempotencyIndex = new Map(this.platformRepository.readState().incidentIdempotencyKeys.map((item) => [item.key, {
            ...
            outcome = await this.platformRepository.runInTransaction(`platform:incident:${incident.id}`, async (repository) => {
                ...
                if (cached) {
                    return cached.fingerprint === fingerprint
                        ? { kind: "duplicate", result: clone(cached.result) }
                        : { kind: "conflict" };
                }
...
function buildIncidentUpdateFingerprint(incidentId, request) {
...
async function incidentDetailPayload(incident, platformRepository) {
...
function incidentTraceId(operation) {
...
function isSupportedIncidentStatus(status) {
...
function realtimeIncidentEvent(incident, traceId) {
    return {
        data: {
            incidentId: incident.id,
