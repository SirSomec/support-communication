...
import { WorkspaceRepository, TemplateOwnershipConflictError } from "./workspace.repository.js";
...
export class WorkspaceService {
    ...
    async fetchClientSegments(context = {}) {
    ...
    async createClientExport(payload = {}, context = {}) {
    ...
    async mergeClientProfiles(payload, context = {}) {
        if (!hasReason(payload.reason)) {
    ...
    async unmergeClientProfile(payload, context = {}) {
        if (!hasReason(payload.reason)) {
    ...
    async createUploadDescriptor(payload, context = {}) {
    ...
    async finalizeUpload(payload, context = {}) {
    ...
    async recordScanResult(payload, context = {}) {
    ...
    async getDownloadPolicy(fileId, context = {}) {
    ...
    async fetchTemplates(filters = {}, context = {}) {
    ...
    async saveTemplate(template, context = {}) {
        if (!context.tenantId) {
        ...
        const auditEvent = await this.workspaceRepository.saveTemplateAuditEvent({
    ...
    async fetchKnowledgeArticles(filters = {}, context = {}) {
    ...
    async fetchKnowledgeArticle(articleId, context = {}) {
    ...
    async createKnowledgeArticle(payload, context = {}) {
        const tenantId = String(context.tenantId ?? "").trim();
        const title = normalizeText(payload.title);
        if (!tenantId)
            return tenantContextRequiredEnvelope(KNOWLEDGE_SERVICE, "createKnowledgeArticle", {});
        if (!title)
            return invalidEnvelope(KNOWLEDGE_SERVICE, "createKnowledgeArticle", "knowledge_article_title_required", "Knowledge article title is required.", {});
    ...
    async saveKnowledgeArticleDraft(payload, context = {}) {
        if (!hasReason(payload.reason)) {
    ...
    async submitKnowledgeArticleForReview(payload, context = {}) {
    ...
    async approveKnowledgeArticle(payload, context = {}) {
        return this.transitionKnowledgeArticle(payload, context, {
            action: "approved",
    ...
    async publishKnowledgeArticle(payload, context = {}) {
        ...
        if (result.status === "ok") {
            ...
            const published = result.data?.article;
            const tenantId = String(context.tenantId ?? "").trim();
    ...
    async rejectKnowledgeArticle(payload, context = {}) {
        return this.transitionKnowledgeArticle(payload, context, {
    ...
    async archiveKnowledgeArticle(payload, context = {}) {
        return this.transitionKnowledgeArticle(payload, context, {
            action: "archived",
    ...
    async addKnowledgeArticleAttachment(payload, context = {}) {
        if (!hasReason(payload.reason)) {
    ...
    async deleteKnowledgeArticleAttachment(payload, context = {}) {
        if (!hasReason(payload.reason)) {
    ...
    async findArticle(articleId, context = {}) {
        const articles = await this.listKnowledgeArticles(context);
        return articles.find((article) => article.id === articleId);
    }
    async transitionKnowledgeArticle(payload, context, transition) {
    ...
    async prepareKnowledgeWorkflow(payload, context, operation, allowedStatuses) {
        if (!hasReason(payload.reason)) {
            return {
                ok: false,
    ...
    async persistKnowledgeTransition(article, payload, context, transition) {
        const now = new Date().toISOString();
...
export function normalizeTemplateScope(value) {
    ...
    if (["personal", "private", "личный"].includes(raw))
        return "personal";
    if (["team", "shared", "командный"].includes(raw))
        return "team";
    if (["global", "глобальный"].includes(raw))
        return "global";
    return undefined;
}
export function hasSharedTemplateAccess(permissions) {
...
function isTemplateVisibleInContext(template, context) {
...
function buildClientSegments(profiles) {
    return [
...
function countClientSegmentDimension(profiles, dimension, valueSelector) {
...
function resolveClientSegment(profiles, segmentId) {
...
function parseClientSegmentId(segmentId) {
...
function filterClientProfilesBySegment(profiles, segment) {
    if (!segment) {
        return profiles;
    }
...
function normalizeClientExportFormat(format) {
    return String(format ?? "").trim().toLowerCase() === "csv" ? "csv" : "json";
}
function clientExportFileName(segment, format) {
...
function maskClientProfileForExport(profile) {
    return {
...
function detectConflict(profiles, primaryProfileId, candidateProfileId) {
...
function tenantContextRequiredEnvelope(service, operation, data = {}) {
    return invalidEnvelope(service, operation, "tenant_context_required", "A current tenant is required for this operation.", data);
}
...
function deniedEnvelope(service, operation, code, message, data) {
    return createEnvelope({
        service,
        operation,
        traceId: workspaceTraceId(service, operation),
...
function fileFinalizeAuditEvent(file, action) {
    return {
        id: file.auditId,
        action,
        fileId: file.fileId,
...
function uploadDescriptorDeniedAuditEvent(reason, channel, tenantId) {
    return {
        id: makeAuditId("file"),
...
function createRequestFingerprint(scope, value) {
...
function scanStateForVerdict(verdict) {
    if (verdict === "clean") {
        return "scan_clean";
    }
    if (verdict === "infected") {
...
function knowledgeActor(actor) {
    return normalizeText(actor) ?? "current-service-admin";
}
function knowledgeAttachmentId(attachment) {
    return String(attachment.id ?? attachment.fileId ?? attachment.name ?? "").trim();
}
function normalizeKnowledgeAttachment(attachment) {
...
function isKnowledgeAttachmentPublishable(attachment) {
...
function knowledgeDecisionHistoryEvent(decision, tone) {
...
function updateKnowledgeVersionStatuses(article, status, updated, author, changes) {
    ...
    return versions.map((version, index) => (index === 0
        ? {
            ...version,
            status,
            author,
...
function isKnowledgeArticleVisible(article, filters) {
...
function uploadDescriptor(file, signedUpload) {
    return {
        fileId: file.fileId,
...
function workspaceTraceId(service, operation) {
