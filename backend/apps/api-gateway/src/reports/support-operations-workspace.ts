...
const METRIC_DEFINITIONS: readonly SupportOperationsMetricDefinition[] = [
  definition("incoming", "count", "Уникальные обращения с известным временем создания, попавшим в выбранный период.", ["Время создания обращения", "Событие создания обращения"], ["Строки без корректного времени создания исключаются."]),
  definition("resolved", "count", "Уникальные обращения хотя бы с одним зафиксированным закрытием или решением в выбранном периоде.", ["Изменение статуса", "Событие закрытия обращения", "Событие решения обращения", "Время закрытия", "Время обновления"], ["Время обновления используется только как запасной вариант для обращений, которые сейчас закрыты."]),
  definition("backlog", "count", "Обращения, созданные до момента снимка и не находящиеся в зафиксированном закрытом состоянии на этот момент.", ["Время создания обращения", "История статусов", "Время закрытия"], ["Исторический бэклог точен только при наличии истории переходов статусов."]),
  definition("waiting", "count", "Открытые обращения из бэклога, у которых последнее видимое клиенту сообщение на момент снимка отправил клиент.", ["Сторона сообщения", "Тип сообщения", "История сообщений"], ["Ответы бота не снимают статус ожидания ответа сотрудника."]),
  definition("slaAttainmentPercent", "percent", "100 × (зафиксированные случаи SLA − нарушения SLA) / зафиксированные случаи SLA.", ["История SLA", "Текущий индикатор SLA"], ["Без зафиксированных данных SLA возвращается «—»; текущий индикатор SLA используется как запасной вариант."]),
  definition("slaBreaches", "count", "Входящие обращения с явной записью нарушения SLA или критичным индикатором SLA.", ["Просрочка SLA", "Нарушение SLA", "Текущий индикатор SLA"], ["Считаются уникальные обращения, а не число событий нарушения."]),
  definition("slaRecordedSamples", "count", "Входящие обращения с явным признаком выполненного или нарушенного SLA.", ["История SLA", "Текущий индикатор SLA"], ["Индикаторы «под угрозой» и «неизвестно» исключаются."]),
  definition("firstResponseAverageSeconds", "seconds", "Среднее арифметическое от первого сообщения клиента до первого следующего публичного ответа сотрудника по входящим обращениям.", ["Сообщения", "Входящее сообщение", "Отправленное сообщение"], ["Внутренние, системные, CSAT- и явно бот-сообщения исключаются."]),
  definition("firstResponseMedianSeconds", "seconds", "P50 по зафиксированным длительностям первого ответа с линейной интерполяцией.", ["Сообщения", "История сообщений"], ["Без зафиксированного ответа сотрудника возвращается «—»."]),
  definition("firstResponseP90Seconds", "seconds", "P90 по зафиксированным длительностям первого ответа с линейной интерполяцией.", ["Сообщения", "История сообщений"], ["На малой выборке процентиль может заметно колебаться."]),
  definition("firstResponseSamples", "count", "Входящие обращения с измеримым первым ответом сотрудника.", ["Сообщения", "История сообщений"], []),
  definition("firstResponseCoveragePercent", "percent", "100 × измеримые первые ответы / входящие обращения.", ["Время создания обращения", "Сообщения"], ["Низкое значение говорит о неполных данных ответов или об обращениях без ответа."]),
  definition("nextResponseMedianSeconds", "seconds", "P50 от каждого не первого сообщения клиента, начавшегося в периоде, до следующего публичного ответа сотрудника.", ["Сообщения", "История сообщений"], ["Несколько сообщений клиента подряд до ответа считаются одним ходом клиента."]),
  definition("nextResponseP90Seconds", "seconds", "P90 от каждого не первого сообщения клиента, начавшегося в периоде, до следующего публичного ответа сотрудника.", ["Сообщения", "История сообщений"], ["Сообщения без ответа не считаются длительностями и остаются в показателях ожидания и бэклога."]),
  definition("nextResponseSamples", "count", "Измеримые не первые ходы клиента, начавшиеся в периоде.", ["Сообщения", "История сообщений"], []),
  definition("firstResolutionMedianSeconds", "seconds", "P50 от известного момента создания обращения до первого зафиксированного решения для обращений, решённых в периоде.", ["Время создания обращения", "История решений"], ["Без времени создания или решения возвращается «—»."]),
  definition("firstResolutionP90Seconds", "seconds", "P90 от известного момента создания обращения до первого решения для обращений, решённых в периоде.", ["Время создания обращения", "История решений"], []),
  definition("firstResolutionSamples", "count", "Решённые обращения с измеримыми временем создания и временем первого решения.", ["Время создания обращения", "История решений"], []),
  definition("fullResolutionMedianSeconds", "seconds", "P50 от создания до последнего решения перед снимком для решённых обращений, которые остаются закрытыми на момент снимка.", ["Время создания обращения", "История решений и переоткрытий"], ["Переоткрытые и оставшиеся открытыми на момент снимка обращения исключаются."]),
  definition("fullResolutionP90Seconds", "seconds", "P90 от создания до последнего устойчивого решения перед моментом снимка.", ["Время создания обращения", "История решений и переоткрытий"], []),
  definition("fullResolutionSamples", "count", "Решённые обращения с измеримым устойчивым решением на момент снимка.", ["История решений и переоткрытий"], []),
  definition("reopenedConversations", "count", "Обращения, решённые в периоде и затем переоткрытые до момента снимка.", ["Событие переоткрытия обращения", "Изменение статуса"], ["Нужны данные о переходах к переоткрытию."]),
  definition("reopenRatePercent", "percent", "100 × переоткрытые обращения / уникальные обращения, решённые в периоде.", ["История решений и переоткрытий"], ["Без решённых обращений возвращается «—»."]),
  definition("oneTouchResolutionCount", "count", "Решённые в периоде обращения ровно с одним публичным сообщением сотрудника до решения и без зафиксированного переоткрытия.", ["Сообщения", "История решений"], ["Это рабочий прокси-показатель, а не доказательство, что вопрос потребовал только одного взаимодействия."]),
  definition("oneTouchResolutionPercent", "percent", "100 × число обращений, решённых с одного касания / уникальные обращения, решённые в периоде.", ["Сообщения", "История решений"], ["Без решённых обращений возвращается «—»."]),
  definition("oneTouchResolutionSamples", "count", "Уникальные обращения, решённые в периоде; используются как знаменатель показателя «решено с одного касания».", ["История решений"], []),
  definition("csatAverage", "score", "Среднее арифметическое последних корректных оценок по обращениям, решённым в периоде, если все оценки в одной шкале.", ["Проверка качества", "Оценка", "Оценки", "История CSAT"], ["Без оценок или при смешанных шкалах возвращается «—»."]),
  definition("csatPositiveRatePercent", "percent", "100 × положительные корректные оценки / корректные оценки с известной шкалой.", ["Проверка качества", "Оценка", "Оценки"], ["Положительная оценка: ≥4/5, ≥9/10, положительная бинарная оценка или ≥80 % для больших шкал."]),
  definition("csatSamples", "count", "Решённые в периоде обращения с последней корректной оценкой до момента снимка.", ["Проверка качества", "Оценка", "Оценки"], ["Используется не более одной последней оценки на обращение."]),
  definition("csatCoveragePercent", "percent", "100 × решённые обращения с оценкой / уникальные обращения, решённые в периоде.", ["Оценки", "История решений"], ["Это прокси покрытия когорты: в исходной строке не всегда есть запись об отправке опроса."]),
  definition("csatScaleMaximum", "score", "Общий максимум шкалы, если все оценки CSAT используют одну известную шкалу.", ["Шкала оценки"], ["Без известной или при смешанных шкалах возвращается «—»."]),
  definition("internalComments", "count", "Внутренние сообщения, созданные в периоде и не принадлежащие боту.", ["Внутренние сообщения", "Признак внутреннего сообщения"], ["При автоматических внутренних сообщениях показатель зависит от отметок автора-бота."]),
  definition("agentTouches", "count", "Публичные сообщения сотрудников-людей, созданные в периоде, без сообщений бота.", ["Сторона сообщения: сотрудник", "Отправленные сообщения"], ["Считаются сообщения, а не уникальные обращения или рабочие сессии."])
];
