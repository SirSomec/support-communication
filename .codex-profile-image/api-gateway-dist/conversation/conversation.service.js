...
import { ConversationAssignmentConflictError, ConversationRepository } from "./conversation.repository.js";
import { createDisabledRealtimeFanoutAdapter } from "./realtime.fanout.js";
...
function normalizeClientPhoneInput(value) {
...
function redactBotHandoffFields(handoff) {
    if (!handoff)
        return handoff;
...
function redactSensitiveConversationFields(conversation, scope) {
    if (scope.canViewSensitive)
        return conversation;
    return {
        ...conversation,
...
function botHandoffFromLifecycleEvents(events) {
...
function normalizeConversationTag(value) {
    return String(value ?? "").trim().replace(/\s+/g, " ").toLowerCase();
}
function isServiceConversationTag(tag) {
...
let defaultOutboundMessageDispatcher = {
    deliverMessage() {
        return { status: "skipped", reason: "outbound_dispatcher_not_configured" };
    }
};
function createDefaultAttachmentStorage() {
    return {
...
export class ConversationService {
    ...
    lastRealtimeOccurredAtMs = 0;
    liveRealtimeEvents = [];
    outboundMessageDispatcher;
    realtimeFanout;
    constructor(conversationRepository = ConversationRepository.default(), options = {}) {
    ...
    static useDefaultRealtimeFanout(adapter) {
        defaultRealtimeFanout = adapter;
    }
    static useDefaultOutboundMessageDispatcher(dispatcher) {
    ...
    async decorateInboxConversations(items) {
        if (!items.length) {
            return items;
        }
    ...
    async fetchDialogs(filters = {}, scope = {}) {
        const tenantId = String(scope.tenantId ?? "").trim();
        if (!tenantId) {
            return tenantContextRequiredEnvelope(DIALOG_SERVICE, "fetchDialogs", { filters });
        }
    ...
    async fetchDialogDetail(conversationId, scope = {}) {
    ...
    async fetchConversationTimeline(conversationId, filters, scope = {}) {
        const tenantId = String(scope.tenantId ?? "").trim();
        if (!tenantId)
            return tenantContextRequiredEnvelope(DIALOG_SERVICE, "fetchConversationTimeline", { conversationId });
        const conversation = await this.conversationRepository.findConversation(conversationId);
    ...
    async assignConversation(payload, scope = {}) {
    ...
    async transitionConversationStatus(payload, scope = {}) {
        const conversation = await this.conversationRepository.findConversation(payload.conversationId);
        ...
        const lifecycleEvent = this.createLifecycleEvent(previousStatus === nextStatus && previousTopic !== conversation.topic ? "topic.changed" : "status.changed", conversation, event, scope, {
            fromStatus: previousStatus,
    ...
    async updateConversationTags(payload, scope = {}) {
        const conversation = await this.conversationRepository.findConversation(payload.conversationId);
        if (!conversation || !matchesTenantScope(conversation, scope.tenantId)) {
    ...
    async updateConversationClientPhone(payload, scope = {}) {
        const conversation = await this.conversationRepository.findConversation(payload.conversationId);
        if (!conversation || !matchesTenantScope(conversation, scope.tenantId)) {
    ...
    async resolvePublicDeliveryAttachments(attachments, tenantId) {
        const resolved = [];
    ...
    async appendMessage(payload, scope = {}) {
        const conversation = await this.conversationRepository.findConversation(payload.conversationId);
        if (!conversation || !matchesTenantScope(conversation, scope.tenantId)) {
    ...
    async uploadAttachment(payload, scope = {}) {
    ...
    async finalizeAttachmentUpload(payload, scope = {}) {
    ...
    async resolveMessageAuthorName(scope, tenantId) {
    ...
    async fetchAttachmentUploadStatus(fileId, scope = {}) {
    ...
    async createOutboundConversationRequest(payload, scope = {}) {
        const channel = String(payload.channel ?? "").trim();
    ...
    async normalizeInboundEvent(channel, payload) {
        ...
        const message = {
            ...
            ...(csatFeedback ? { type: "csat_feedback" } : {}),
    ...
    async recordDeliveryReceipt(channel, payload, scope = {}) {
    ...
    async fetchRealtimeEvents(filters = {}, scope = {}) {
        const tenantId = String(scope.tenantId ?? "").trim();
        if (!tenantId) {
            return tenantContextRequiredEnvelope(REALTIME_SERVICE, "fetchRealtimeEvents", { filters });
        }
    ...
    async findAttachmentUploadFile(fileId, tenantId) {
    ...
    async appendAndPublishRealtimeEvent(event) {
    ...
    createRealtimeEvent(eventName, resourceType, resourceId, data, tenantId) {
        if (!tenantId) {
    ...
    createLifecycleEvent(eventType, conversation, realtimeEvent, scope, data, reason) {
        return {
...
function conversationTraceId(service, operation) {
...
function createConversationOutboundDescriptor(input) {
    if (!input.tenantId) {
        throw new Error("tenant_context_required");
    }
    return {
...
function createChannelCsatSurvey(conversation, traceId, tenantId) {
...
function providerConversationIdFromConversation(conversation) {
...
function outboundDeliveryFromDescriptor(descriptor) {
    return {
        channel: descriptor.channel,
...
function outboundMessageFromDescriptor(descriptor, fallbackText, fallbackAttachments) {
    ...
    const descriptorAttachments = Array.isArray(descriptor.payload.attachments)
        ? descriptor.payload.attachments.map((attachment) => {
            if (!attachment || typeof attachment !== "object" || Array.isArray(attachment))
                return attachment;
            const { signedFile: _signedFile, ...publicAttachment } = attachment;
...
function attachmentUploadDataFromDescriptor(descriptor) {
    return {
        id: descriptor.id,
...
function attachmentUploadDataFromFile(file, descriptor) {
...
function attachmentFinalizeDeniedEnvelope(file, descriptor, code, message) {
    return createEnvelope({
        service: DIALOG_SERVICE,
        operation: "finalizeAttachmentUpload",
        traceId: conversationTraceId(DIALOG_SERVICE, "finalizeAttachmentUpload"),
...
function attachmentAntivirusState(file) {
...
function createQueuedOutboundConversationRecord(descriptor, fallbackMessage, fallbackClientName, authorName) {
...
function isSignedObjectStorageUrlData(value) {
    return Boolean(value)
        && typeof value === "object"
        && !Array.isArray(value)
        && typeof value.expiresAt === "string"
        && (value.method === "GET" || value.method === "PUT")
...
function signedObjectStorageUrlData(signedUrl) {
    return {
...
function conflictEnvelope(service, operation, code, message, data) {
    return createEnvelope({
        service,
        operation,
...
function invalidEnvelope(service, operation, code, message, data) {
    return createEnvelope({
        service,
        operation,
        traceId: conversationTraceId(service, operation),
...
function notFoundEnvelope(service, operation, code, message, data) {
    return createEnvelope({
        service,
        operation,
        traceId: conversationTraceId(service, operation),
...
function tenantContextRequiredEnvelope(service, operation, data) {
...
function boundedRealtimeLimit(value) {
    return Math.min(500, toPositiveInt(value, 200));
}
function matchesTenantScope(conversation, tenantId) {
    if (!tenantId) {
        return true;
    }
...
function employeeTeamId(metadata) {
    const settings = metadata?.employeeSettings;
    if (!settings || typeof settings !== "object" || Array.isArray(settings)) {
        return undefined;
    }
    const teamId = String(settings.groupId ?? "").trim();
