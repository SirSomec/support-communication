...
- Базовые стили находятся в `src/styles.css`; shared Toast/StatusBadge/Permission/ChannelBadge/ToolbarSearch/SegmentedControl CSS вынесен в `src/ui.css`, admin Webhooks/API/Security стили настроек вынесены в `src/features/settings/settings.css`, chart/rescue/column controls отчетов — в `src/features/reports/reports.css`, стили notification center — в `src/features/notifications/notifications.css`, CSS модалок диалога — в `src/features/dialogs/dialog-modals.css`, CSS composer surface — в `src/features/dialogs/dialog-composer.css`, CSS bot builder — в `src/features/automation/automation.css`, CSS KnowledgeBaseWorkspace — в `src/features/quality/knowledge-base.css`, CSS AiQualityWorkspace — в `src/features/quality/ai-quality.css`.
...
1. Продолжить разнос `App.jsx`, `styles.css` и крупных feature-экранов на подмодули без изменения поведения; app shell, PanelScreen, ClientsScreen, ReportsScreen, TemplatesScreen, QualityScreen, VisitorsScreen, AutomationScreen, SettingsScreen, notification center, ConversationList, ChatPane, CustomerPanel, DialogModals, composer, DialogActionMenu, AuditTimeline, ChatHeader, TranscriptToolbar, Modal, `useAppTransientState`, `useComposerState`, `useComposerAttachments`, `useDialogQueueFilters`, `useConversationMutations`, `useAppNavigation`, `useAiSuggestions`, `useDialogActions`, `useOutboundConversation`, outbound factory `createOutboundConversation`, reports CSS, notifications CSS, dialog modals CSS, composer CSS, Automation bot builder CSS, KnowledgeBaseWorkspace CSS, AiQualityWorkspace CSS, `SettingsAccessPanel`, admin settings workspace, `EmployeeManagementPanel`, `ChannelConnectionsPanel`, `SdkConsolePanel`, `TopicDirectoryPanel` и `SectionRouter` уже вынесены/переименованы. Следующий безопасный кандидат — дальнейшая декомпозиция feature-экранов или вынос оставшихся global CSS-блоков без изменения сценариев.
2. Реализовано: общий `Modal`; `Toast`, `StatusBadge`, `ToolbarSearch`, `SegmentedControl`, `EntityTable`, `Permission` и `Modal` уже вынесены в `src/ui.jsx`, Toast/StatusBadge/Permission/ChannelBadge/ToolbarSearch/SegmentedControl CSS — в `src/ui.css`, `DialogActionMenu` и `AuditTimeline` — в `src/features/dialogs/*`.
...
- [x] Вынести report chart/rescue/column CSS из `src/styles.css` в `src/features/reports/reports.css`.
