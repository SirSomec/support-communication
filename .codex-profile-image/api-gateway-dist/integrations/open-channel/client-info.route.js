...
export async function handleWidgetClientInfoFromRoute(input) {
...
function applyContactInfo(conversation, contact) {
    let changed = false;
...
function applyPage(conversation, pageUrl, pageTitle) {
...
function appendCustomDataEvent(conversation, customData) {
...
function normalizeAttributes(input) {
    ...
    const entries = Object.entries(input)
        ...
        .slice(0, 50)
        .map(([key, value]) => [String(key).trim().slice(0, 100), typeof value === "number" ? value : String(value ?? "").slice(0, 500)]);
    return entries.length ? Object.fromEntries(entries) : undefined;
}
async function persistClientInfoMutation(repository, conversation) {
...
//# sourceMappingURL=client-info.route.js.map