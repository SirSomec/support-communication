...
const outboxStore: OutboxEventStore = createPrismaOutboxStore(client);
const outboundDescriptorStore: ConversationOutboundDescriptorStore = createPrismaConversationOutboundDescriptorStore(client);
const providerAttachmentTransferStore = createPrismaProviderAttachmentTransferStore(client);
const telegramBotTokenResolver = createPrismaIntegrationTelegramTokenResolver(client, process.env.OUTBOX_TELEGRAM_BOT_TOKEN);
...
try {
  ...
  console.log(JSON.stringify({
    ...
    conversationDeliveryStore: "prisma",
    result
  }));
  }
} finally {
