...
import { disableTelegramConnectionRecord, saveTelegramConnectionRecord, telegramConnectionEnvelope, toTelegramConnectionPublicView } from "./telegram-channel-connection.js";
import { ProviderConnectionCrypto } from "./provider-connection-crypto.js";
import { subscribeMaxWebhook } from "./max-subscription.js";
import { prepareVkCallback, subscribeVkCallback } from "./vk-callback.js";
...
export class IntegrationService {
    ...
    deliveries;
    sessions;
    replayIdempotency = new Map();
    queueDirectoryRepository;
    constructor(integrationRepository = IntegrationRepository.default(), options = {}) {
    ...
    async fetchIntegrationCapabilities() {
        const services = [
    ...
    async fetchChannelConnections(tenantId, filters = {}) {
    ...
    async createChannelConnection(tenantId, payload) {
        const normalizedTenantId = normalizeTenantId(tenantId);
        if (!normalizedTenantId) {
        ...
        if (type === "telegram") {
            ...
            try {
                ...
                return invalidEnvelope("createChannelConnection", code, "Telegram bot token could not be validated.", { type });
            }
        }
        ...
        const providerCrypto = type === "vk" || type === "max"
            ? (this.options.providerCredentialCrypto ?? ProviderConnectionCrypto.fromEnvironment(process.env.PROVIDER_CREDENTIAL_KEY_VERSION?.trim() || "v1"))
            : null;
        let providerCredential = null;
        let webhookSecret = null;
        let vkConfirmationCode = null;
        if (type === "vk" || type === "max") {
    ...
    async updateChannelConnection(tenantId, connectionId, payload) {
        const normalizedTenantId = normalizeTenantId(tenantId);
    ...
    async updateChannelTypeStatus(tenantId, type, payload) {
        const normalizedTenantId = normalizeTenantId(tenantId);
        if (!normalizedTenantId) {
        ...
        if (!connections.length) {
            return notFoundEnvelope("updateChannelTypeStatus", "channel_type_connections_not_found", `Channel type ${normalizedType} has no tenant connections.`, { tenantId: normalizedTenantId, type: normalizedType });
        }
        const now = new Date().toISOString();
        ...
        await Promise.all(savedConnections.map((connection) => this.recordChannelConnectionEvent(connection.tenantId, connection.id, "channel.type_status.updated", payload.enabled ? "info" : "warn", reason)));
        const auditEvents = await Promise.all(savedConnections.map((connection) => this.persistChannelConnectionAuditEvent("channel.type_status.update", connection, reason)));
        return createEnvelope({
            service: INTEGRATION_SERVICE,
    ...
    async deleteChannelConnection(tenantId, connectionId, payload = {}) {
        const normalizedTenantId = normalizeTenantId(tenantId);
        const connection = await this.findChannelConnection(normalizedTenantId, connectionId);
        if (!connection) {
    ...
    async testChannelConnectionInstance(tenantId, connectionId, payload) {
        const normalizedTenantId = normalizeTenantId(tenantId);
        const connection = await this.findChannelConnection(normalizedTenantId, connectionId);
        if (!connection) {
    ...
    async fetchChannelConnectionEvents(tenantId, connectionId) {
        const normalizedTenantId = normalizeTenantId(tenantId);
        const connection = await this.findChannelConnection(normalizedTenantId, connectionId);
        if (!connection) {
    ...
    async createPublicApiKey(payload = {}) {
        const name = String(payload.name ?? "").trim();
        if (!name) {
    ...
    async revokePublicApiKey(keyId) {
    ...
    async resolveWebhookEndpointView(endpointId) {
    ...
    async replayWebhookDelivery(payload) {
    ...
    async fetchTelegramConnection(tenantId) {
    ...
    async saveTelegramConnection(tenantId, payload, options = {}) {
        const normalizedTenantId = String(tenantId ?? "").trim();
    ...
    async disconnectTelegramConnection(tenantId) {
        const normalizedTenantId = String(tenantId ?? "").trim();
        const existing = await this.integrationRepository.findTelegramConnectionByTenantIdAsync(normalizedTenantId);
    ...
    async validateRoutingQueue(tenantId, queueId) {
        if (!queueId) {
    ...
    async findReplay(idempotencyKey) {
    ...
    async recordChannelConnectionEvent(tenantId, connectionId, action, severity, message) {
    ...
    persistChannelConnectionAuditEvent(action, connection, reason) {
        const event = {
            action,
            at: new Date().toISOString(),
            connectionId: connection.id,
...
function resolveTestTelegramFetch(botToken) {
...
function findConnection(channel, connectionId, environment) {
    if (connectionId) {
...
function integrationTraceId(operation) {
...
function fixtureKeyCreatedAt(key) {
    const timestamp = Date.parse(`${key.lastRotated}T00:00:00.000Z`);
    return Number.isNaN(timestamp) ? new Date().toISOString() : new Date(timestamp).toISOString();
}
function fixtureKeyStatus(key) {
    return key.status.toLowerCase() === "revoked" ? "revoked" : "active";
}
function createWebhookReplayAuditEvent({ action, auditId, delivery, idempotencyKey, replayId, transition }) {
    return {
        action,
        at: new Date().toISOString(),
...
function journalEntryToWebhookDelivery(entry) {
    return {
...
function buildWebhookDeliveryReadSide(fixtureDeliveries, journalEntries) {
...
function buildWebhookDeadLetterReadSide(journalEntries) {
...
function webhookReplayTransition(delivery) {
...
function buildApiKeyReadSide(fixtures, stored) {
...
function toApiEnvironmentKeyView(key) {
    return {
        env: key.environment,
        id: key.keyId,
...
function buildWebhookEndpointReadSide(seedEndpoints, records) {
...
function isValidWebhookEndpointUrl(url) {
    try {
...
function normalizePublicApiEnvironment(environment) {
...
function normalizePublicApiKeyScopes(scopes) {
...
function normalizeWebhookEndpointStatus(status) {
...
function maskChannelConnection(connection) {
    return {
...
function summarizeChannelTypeStatus(type, connections, catalog) {
    const activeCount = connections.filter((connection) => connection.status === "active").length;
    const channel = catalog.find((item) => normalizeOptionalType(item.id) === type
...
function isTokenManagedChannelType(type) {
...
function extractCredentialMaterial(credentials) {
...
function resolveChannelServiceEndpoint(type, connectionId, providedWebhookUrl) {
...
function overlaySecuritySessions(base, persisted) {
    if (!persisted.length) {
        return base;
    }
...
function readInitialIntegrationState(repository) {
    return repository.readInitialState();
}
function resolvePublicWebhookBaseUrl(env = process.env) {
