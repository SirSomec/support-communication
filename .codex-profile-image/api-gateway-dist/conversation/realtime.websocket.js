...
export function installRealtimeWebSocketReplay(server, options) {
...
async function handleRealtimeUpgrade(request, socket, socketPath, options) {
...
export async function writeRealtimeWebSocketReplay(conversationService, socket, since, scope = {}, limit) {
...
export async function authorizeRealtimeSocket(headers, config) {
    const authorization = readHeader(headers, "authorization");
    if (/^Bearer\s+/i.test(authorization)) {
        let serviceAdminTenantId;
...
function writeTextFrame(socket, value) {
...
function createLongFrameHeader(length) {
...
//# sourceMappingURL=realtime.websocket.js.map