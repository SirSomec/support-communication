 ...
 */
export const OPENAI_COMPATIBLE_EMBEDDING_PROVIDER_ID = "openai-compatible-embedding";
...
export function createOpenAiCompatibleEmbeddingProvider(connection, options = {}) {
...
export function buildEmbeddingRequestBody(inputs, model) {
    return requestBody(inputs, model);
}
function requestBody(inputs, model) {
    if (!Array.isArray(inputs) || inputs.length === 0)
        throw new Error("ai_embedding_inputs_required");
    if (inputs.length > MAX_INPUTS_PER_CALL)
        throw new Error("ai_embedding_inputs_too_many");
    const input = inputs.map((item) => {
        const text = String(item ?? "").trim();
        if (!text)
            throw new Error("ai_embedding_input_invalid");
 ...
 */
function parseEmbeddings(value, configuredModel, expectedCount) {
...
//# sourceMappingURL=openai-compatible-embedding.provider.js.map