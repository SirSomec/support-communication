...
import { createPrismaBillingSyncQueueSummaryStore, createPrismaClient, createPrismaOutboxQueueSummaryStore } from "@support-communication/database";
...
import { runDeadLetterReplayWorker, runMigrationRollbackTooling, runQueuedLoadTestExecution, runRestoreDrillWorkers } from "./operations-runtime.js";
...
export class OperationsReadinessService {
    ...
    loadTestIdempotency;
    restoreCheckIdempotency;
    deadLetterIdempotency;
    constructor(operationsRepository = OperationsRepository.default(), integrationRepository = IntegrationRepository.default(), notificationRepository = NotificationRepository.default(), reportRepository = ReportRepository.default(), queueObservabilitySource = createDefaultQueueObservabilitySource(), automationRepository = AutomationRepository.default()) {
        this.operationsRepository = operationsRepository;
        this.integrationRepository = integrationRepository;
        this.notificationRepository = notificationRepository;
        this.reportRepository = reportRepository;
        this.queueObservabilitySource = queueObservabilitySource;
    ...
    async fetchReadinessDashboard(filters = {}) {
        ...
        const workerObservability = await buildWorkerObservability(this.integrationRepository, this.notificationRepository, this.reportRepository, this.queueObservabilitySource, this.automationRepository);
        const blockers = [
    ...
    async queueLoadTestRun(payload) {
        ...
        const persistedResult = await this.operationsRepository.saveLoadTestRunAsync(result);
        if (idempotencyKey) {
    ...
    async queueRestoreCheck(payload) {
        const request = payload ?? {};
        ...
        const persistedResult = await this.operationsRepository.saveRestoreCheckAsync(result);
        if (idempotencyKey) {
    ...
    async checkMigrationRollback(payload) {
        const request = payload ?? {};
    ...
    async fetchDeadLetterDashboard(filters = {}) {
    ...
    async replayDeadLetterMessage(payload) {
        const request = payload ?? {};
        ...
        if (workerResult.status === "denied") {
            ...
            return conflictEnvelope("replayDeadLetterMessage", code, workerResult.envelope?.message ?? "Dead-letter replay was denied.", {
    ...
    async fetchSecurityReview(filters = {}) {
...
async function findPersistedDeadLetterReplay(operationsRepository, replayId) {
    return operationsRepository.findDeadLetterReplayAsync(replayId);
}
function readInitialOperationsState(operationsRepository) {
    try {
...
let cachedDefaultQueueObservabilitySource = null;
function createDefaultQueueObservabilitySource() {
...
function emptyQueueObservabilitySource() {
    return {
        async summarizeBillingSyncQueue({ queue }) {
            return {
                deadLetterCount: 0,
                latestJob: null,
                queue,
        ...
        async summarizeOutboxQueue({ queue }) {
            return {
                deadLetterCount: 0,
                latestEvent: null,
                queue,
...
async function buildWorkerObservability(integrationRepository, notificationRepository, reportRepository, queueObservabilitySource, automationRepository) {
    const [webhookDeliveryJournal, leadNotificationSummary, browserPushDescriptors, reportExportJobs, scheduledDigestDescriptors, outboxSummary, fileScanSummary, billingSyncSummary, proactiveDeliveryAttempts] = await Promise.all([
...
function buildProactiveDeliveryWorkerObservability(attempts) {
...
function buildWebhookDeliveryWorkerObservability(journal) {
...
function deliveryJournalTimestamp(entry) {
    return entry.lastAttemptAt ?? entry.lockedAt ?? entry.nextAttemptAt ?? entry.createdAt;
}
function buildLeadNotificationWorkerObservability(summary) {
...
function leadNotificationTimestamp(descriptor) {
    return descriptor.payload.delivery?.failedAt
        ?? descriptor.payload.delivery?.deliveredAt
        ?? descriptor.createdAt;
}
function buildNotificationDeliveryWorkerObservability(descriptors) {
...
function notificationDeliveryTimestamp(descriptor) {
...
function buildReportExportWorkerObservability(jobs) {
...
function buildReportDigestWorkerObservability(descriptors) {
...
function buildOutboxWorkerObservability(summary) {
    return buildOutboxQueueWorkerObservability(summary, "outbox-worker");
}
function buildFileScanScannerWorkerObservability(summary) {
    return buildOutboxQueueWorkerObservability(summary, "file-scan-scanner-worker");
}
function buildOutboxQueueWorkerObservability(summary, workerId) {
...
function buildBillingSyncWorkerObservability(summary) {
...
function billingSyncQueueTimestamp(job) {
...
function billingSyncEventType(job) {
...
function workerQueueHealth(queueDepth, deadLetterCount) {
...
function auditEvent(action, target, reason, actor) {
    return {
...
function isSupportedSecurityArea(area) {
...
function okEnvelope(operation, data, meta = {}) {
    return createEnvelope({
        service: OPERATIONS_SERVICE,
        operation,
        traceId: operationsTraceId(operation),
...
//# sourceMappingURL=operations-readiness.service.js.map