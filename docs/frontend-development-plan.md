...
- Продуктовые разделы находятся в `src/sections.jsx`, но `PanelScreen` уже вынесен в `src/features/panel/PanelScreen.jsx`, `ClientsScreen` — в `src/features/clients/ClientsScreen.jsx`, `ReportsScreen` — в `src/features/reports/ReportsScreen.jsx`, `TemplatesScreen` — в `src/features/templates/TemplatesScreen.jsx`, `QualityScreen` — в `src/features/quality/QualityScreen.jsx`, `VisitorsScreen` — в `src/features/visitors/VisitorsScreen.jsx`, а `AutomationScreen` — в `src/features/automation/AutomationScreen.jsx`; маршрутизация разделов вынесена в `src/features/section-router.jsx`, расширенный workspace базы знаний вынесен в `src/features/quality/KnowledgeBaseWorkspace.jsx`, а AI scoring/coaching workspace — в `src/features/quality/AiQualityWorkspace.jsx`.
...
- Helper `createScreenStateItems` вынесен в `src/app/screenState.js` для переиспользования product-screen state strip при дальнейшем разборе `sections.jsx`.
...
1. Продолжить разнос `App.jsx`, `sections.jsx` и `styles.css` по feature-модулям без изменения поведения; app shell, PanelScreen, ClientsScreen, ReportsScreen, TemplatesScreen, QualityScreen, VisitorsScreen, AutomationScreen, notification center, ConversationList, ChatPane, CustomerPanel, DialogModals, composer, DialogActionMenu, AuditTimeline, ChatHeader, TranscriptToolbar и Modal уже вынесены, следующий безопасный кандидат — декомпозиция SettingsScreen из `sections.jsx` или CSS-разделение для стабильных feature-блоков.
