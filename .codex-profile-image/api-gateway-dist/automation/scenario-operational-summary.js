...
export function canViewAiUsage(viewer = {}) {
    if (viewer.isServiceAdmin)
        return true;
    const permissions = viewer.permissions ?? [];
    if (permissions.includes("*"))
        return true;
    return permissions.some((permission) => ["ai.connections.manage", "settings.manage", "settings.write", "automation.write"].includes(permission));
}
export function estimateAiCostBucket(usedTokens) {
    if (usedTokens <= 0)
        return "none";
...
export function estimateAiCostUsd(usedTokens) {
...
export function buildTenantAiUsageSummary(input) {
    if (!canViewAiUsage(input.viewer))
        return null;
...
export function buildScenarioOperationalSummariesFromState(state, tenantId, aiUsage = null) {
...
function resolveStepScenarioId(step, instanceById) {
...
function collectFailures(steps, instances) {
...
function collectHandoffs(steps, instances) {
    const fromSteps = steps
...
function collectLatestCitations(steps) {
    for (const step of steps) {
        ...
        const fromResponse = step.handoffSummary == null
            ? extractCitationsFromUnknown(step.aiResponse)
            : [];
...
function collectLatestFallbackReason(steps, instances) {
    for (const step of steps) {
...
function extractCitationsFromUnknown(value) {
    ...
    return value
        ...
        if (!sourceId || !title || !Number.isFinite(version))
            return null;
...
//# sourceMappingURL=scenario-operational-summary.js.map