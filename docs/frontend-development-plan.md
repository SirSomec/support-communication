...
- Общие UI-примитивы, Toast, StatusBadge, ToolbarSearch и SegmentedControl вынесены в `src/ui.jsx`.
...
2. Вынести общие компоненты `Modal`, `EntityTable`, `AuditTimeline`, `ActionMenu`; `Toast`, `StatusBadge`, `ToolbarSearch` и `SegmentedControl` уже вынесены в `src/ui.jsx`.
