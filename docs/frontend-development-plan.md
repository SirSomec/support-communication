...
2. Реализовано: общий `Modal`; `Toast`, `StatusBadge`, `ToolbarSearch`, `SegmentedControl`, `EntityTable` и `Modal` уже вынесены в `src/ui.jsx`, Toast/StatusBadge CSS — в `src/ui.css`, `DialogActionMenu` и `AuditTimeline` — в `src/features/dialogs/*`.
