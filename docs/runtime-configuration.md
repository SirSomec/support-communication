...
> **Статус на 2026-08-13:** production развёрнут непосредственно на RUVDS и обслуживается `deploy/compose/compose.production.yml` с Caddy edge. `supportcom.ru` резолвится в `194.87.206.159`; `/api/v1/ready` на этом адресе подтверждает доступность API, PostgreSQL, Redis, object storage и почты. Реверс-SSH-туннель, socat bridge и Raspberry Pi не используются. Отчёт [`infrastructure-audit-2026-07-28.md`](infrastructure-audit-2026-07-28.md) описывает прежнее состояние и остаётся историческим источником.
...
- SMTP delivery: `PUBLIC_DEMO_NOTIFICATION_PROVIDER_MODE=smtp` with `PUBLIC_DEMO_NOTIFICATION_SMTP_HOST` or `MAIL_HOST`, `PUBLIC_DEMO_NOTIFICATION_SMTP_PORT` or `MAIL_PORT`, `PUBLIC_DEMO_NOTIFICATION_SMTP_FROM`, and `PUBLIC_DEMO_NOTIFICATION_SMTP_TO`. Set multiple recipients as a comma-separated list, for example `sales@supportcom.ru,admin@supportcom.ru`; the worker sends one SMTP envelope with a `RCPT TO` command for each address. External SMTP credentials are optional but must be supplied as a pair through `PUBLIC_DEMO_NOTIFICATION_SMTP_USERNAME`/`PUBLIC_DEMO_NOTIFICATION_SMTP_PASSWORD` or `MAIL_USERNAME`/`MAIL_PASSWORD`; when present, the worker authenticates with `AUTH PLAIN` before delivery. Implicit TLS for SMTPS endpoints is enabled with `PUBLIC_DEMO_NOTIFICATION_SMTP_SECURE=true`; self-signed staging endpoints can set `PUBLIC_DEMO_NOTIFICATION_SMTP_TLS_REJECT_UNAUTHORIZED=false`.
...
- External channel delivery still requires configured provider env such as `OUTBOX_CHANNEL_CONNECTORS` or `OUTBOX_TELEGRAM_ENABLED=true`. In production-like Prisma mode, Telegram bot tokens resolve from active `telegram_connections` rows by tenant, with `OUTBOX_TELEGRAM_BOT_TOKEN` as an env fallback. Set `OUTBOX_HTTP_TIMEOUT_MS` for the `message-delivery` worker when a provider needs more than the generic 5-second request timeout; production defaults it to 15 seconds.
