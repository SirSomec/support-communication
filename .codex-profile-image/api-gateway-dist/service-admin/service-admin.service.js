...
import { IdentityRepository, isActiveServiceAdminImpersonationConflict } from "../identity/identity.repository.js";
import { applyAuditRedactionOverlay, AUDIT_EXPORT_COLUMNS, AUDIT_EXPORT_REDACTION_POLICY, buildAuditExportDescriptor, createAuditExportRecord, createAuditRedactionRecord, isAuditExportExpired, stableAuditExportFilters, toAuditExportPayloadRow } from "./service-admin-audit.persistence.js";
...
export class ServiceAdminService {
    ...
    async requestAuditExport(filters = {}, actor = undefined) {
        const auditEvents = await this.fetchAuditEvents(filters);
        const items = auditEvents.data.items;
        ...
        const reusable = existingExports.find((record) => (record.descriptorId === descriptorId
            && stableExportFiltersMatch(record.filters, stableAuditExportFilters(filters))
            && !isAuditExportExpired(record)));
        const exportRecord = reusable ?? await this.identityRepository.recordServiceAdminAuditExport(createAuditExportRecord({
            descriptor,
            filters,
    ...
    async redactAuditEvent(payload = {}) {
        ...
        const redaction = await this.identityRepository.recordServiceAdminAuditRedaction(createAuditRedactionRecord({
    ...
    async requestBreakGlassApproval(payload) {
        const request = payload ?? {};
    ...
    async decideBreakGlassApproval(payload) {
        const request = payload ?? {};
    ...
    async applyUserAction(payload, action, mutate, severity = "warning") {
        const request = payload ?? {};
    ...
    async loadAuditRedactionOverlays() {
    ...
    async findActiveImpersonation(tenantId, userId) {
        return this.identityRepository.findActiveServiceAdminImpersonation({
            tenantId,
            userId
        });
    }
    async createImpersonationWithConflictEnvelope(request, impersonation, auditEvent, requestedMode, approvalId) {
        try {
    ...
    async validateBreakGlassWriteApproval(request, tenantId, userId) {
    ...
    async writeApprovalDenied(code, message, request, approval, data) {
        const auditRecord = await this.recordAudit("startImpersonation", {
            action: "impersonation.start",
            actor: request.actor,
            reason: request.reason,
    ...
    async notFoundWithAudit(operation, code, message, data, auditInput) {
    ...
    async recordAudit(operation, { actor, action, idScope = "service_admin", reason, result, severity, target, tenantId, userId }) {
    ...
    buildAuditEvent({ actor, action, idScope = "service_admin", reason, result, severity, target, tenantId, userId }) {
        return {
    ...
    async validatePrivilegedRequest(operation, payload, auditContext) {
...
function actionToOperation(action) {
    ...
    return operations[action] ?? action.replace(/[^a-zA-Z0-9]+(.)/g, (_match, char) => char.toUpperCase());
}
...
function isAfterAuditCursor(event, cursor) {
...
function normalizeAuditLimit(limit, fallback) {
...
function buildAuditExportEnvelope(filters, items, descriptor) {
    return createEnvelope({
        service: SUPPORT_ADMIN_SERVICE,
...
function stableExportFiltersMatch(left, right) {
...
function breakGlassDecisionAccess(approval) {
...
function impersonationAccess(session) {
...
function serviceAdminPersistenceErrorEnvelope(operation, action, auditEvent, data) {
    return createEnvelope({
        service: SUPPORT_ADMIN_SERVICE,
        operation,
        traceId: supportTraceId(operation),
...
function normalizeBreakGlassDecision(decision) {
...
function requestedImpersonationMode(request) {
...
function supportTraceId(operation) {
    return getCurrentTraceId() ?? createRequestTraceId(SUPPORT_ADMIN_SERVICE, operation);
}
function toServiceAdminTenant(tenant) {
    return {
        id: tenant.id,
        name: tenant.name,
        planId: "planId" in tenant ? String(tenant.planId ?? "business") : "business",
        status: tenant.status
    };
}
function toServiceAdminUser(user) {
    return {
        device: user.device,
        email: user.email,
        id: user.id,
...
function toTenantUserChanges(previous, next) {
    const changes = {};
    if (next.device !== previous.device)
        changes.device = next.device;
    if (next.email !== previous.email)
        changes.email = next.email;
    if (next.inviteStatus !== previous.inviteStatus)
        changes.inviteStatus = next.inviteStatus;
    if (next.lastActiveAt !== previous.lastActiveAt)
        changes.lastActiveAt = next.lastActiveAt === "never" ? null : next.lastActiveAt;
    if (next.mfa !== previous.mfa)
        changes.mfa = next.mfa;
    if (next.name !== previous.name)
        changes.name = next.name;
    if (next.risk !== previous.risk)
        changes.risk = next.risk;
    ...
    if (next.supportNotes !== previous.supportNotes)
        changes.supportNotes = next.supportNotes;
    if (next.tenantId !== previous.tenantId)
        changes.tenantId = next.tenantId;
    return changes;
}
//# sourceMappingURL=service-admin.service.js.map