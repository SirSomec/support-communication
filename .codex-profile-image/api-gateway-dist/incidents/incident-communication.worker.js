...
export function planCustomerVisibleIncidentCommunication(input) {
    ...
    const idempotencyKey = makeIncidentCommunicationIdempotencyKey("customer-visible", input.incident.id, input.job.scope, input.job.id);
...
export function planInternalIncidentCommunication(input) {
    ...
    const idempotencyKey = makeIncidentCommunicationIdempotencyKey("internal-only", input.incident.id, input.job.scope, input.job.id);
    const descriptorPayload = {
        incidentId: input.incident.id,
        public: false,
        severity: input.incident.severity,
...
function makeInternalIncidentCommunicationId(incidentId, jobId) {
    return `incident_internal_${sanitizeIdentifierSegment(incidentId)}_${createHash("sha256")
        .update(`internal:${incidentId}:${jobId}`)
        .digest("hex")
        .slice(0, 12)}`;
}
export function persistIncidentCommunicationAttempt(input) {
...
export function resolveIncidentCommunicationFailureState(input) {
...
export function recordIncidentCommunicationRetryState(input) {
...
export function recordIncidentCommunicationDeadLetterState(input) {
    const current = findClaimedIncidentCommunicationAttempt(input.repository, input.attemptId);
...
function findClaimedIncidentCommunicationAttempt(repository, attemptId) {
...
//# sourceMappingURL=incident-communication.worker.js.map